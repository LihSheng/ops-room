...
async function ensureDir(dir) {
  try {
    ...
    if (err.code !== 'EEXIST') console.error(`  Γ£ù ${dir}: ${err.message}`);
  }
}
...
export async function main({
  ...
  if (await loadBootstrapEnvironment(envPath)) {
    output.log('  Γ£ô .env exists and was loaded');
  } else {
    output.log('  Γ£ù .env not found ΓÇö copy .env.example ΓåÆ .env');
  }
  ...
  for (const name of STARTUP_REQUIRED_VARS) {
    output.log(`  ${missing.includes(name) ? 'Γ£ù' : 'Γ£ô'} ${name} is ${missing.includes(name) ? 'NOT set' : 'set'}`);
  }

  if (missing.length > 0) {
    output.error(`\nStartup blocked: ${missing.join(', ')} must be set.`);
    output.error('  Ops Room refuses to start without an explicit webhook bearer secret.');
    output.log('  Copy .env.example ΓåÆ .env and fill in the values.');
    return 1;
  }
