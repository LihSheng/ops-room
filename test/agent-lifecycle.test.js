...
import { readAgentLifecycleState, recoverInterruptedAgentLifecycleStates, updateAgentLifecycleState, } from '../src/services/agent-lifecycle-store.js';
...
test('different idempotency keys serialize without a second runtime stop', async () => {
    ...
    assert.equal(stopCalls, 1);
    assert.deepEqual([first.body.command_executed, second.body.command_executed].sort(), [false, true]);
    const events = await listAuditEvents({ dir: dirs.auditDir, operation: 'agent.stop' });
...
test('docker lifecycle controller uses fixed argv and exposes no force operations', async () => {
    ...
    await assert.rejects(() => controller.stop({ agent_id: 'bad', target: { kind: 'docker-container', name: 'bad;name' } }), /does not support/);
});
