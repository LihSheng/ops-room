...
test('operational API requests require pre-route dashboard authorization', async () => {
    ...
    if (authorized.ok)
        assert.equal(authorized.auth_method, 'dashboard_token');
    const response = captureJsonResponse({ url: '/api/health', authorization: 'Bearer security-test-dashboard' }, { ready: true });
...
for (const route of PROTECTED_ROUTES) {
    ...
    test(`${route}: correct dashboard token authorizes the read`, async () => {
        ...
        if (result.ok)
            assert.equal(result.auth_method, 'dashboard_token');
    });
}
//# sourceMappingURL=security-hardening.test.js.map