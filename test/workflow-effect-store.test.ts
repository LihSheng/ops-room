...
import {
  ...
  listWorkflowEffects,
  readWorkflowEffect,
  reconcileInterruptedWorkflowEffects,
...
function claimInput(dir: string, overrides: any = {}) {
  return {
    dir,
...
test('concurrent claims elect exactly one external executor', async () => {
  ...
  const claims = await Promise.all(
    Array.from({ length: 8 }, () => claimWorkflowEffect(claimInput(dir))),
  );
...
test('same identity with a conflicting payload fails closed', async () => {
  ...
  await assert.rejects(
    claimWorkflowEffect(claimInput(dir, {
