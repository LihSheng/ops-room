...
import { classifyConvergence, isApprovedHealth, readAgentLifecycleState, recoverInterruptedAgentLifecycleStates, updateAgentLifecycleState, agentLifecycleAllowsDispatch, } from '../src/services/agent-lifecycle-store.js';
...
test('rejected starts are audited as agent.start, not agent.stop', async () => {
    ...
    const startRejectAsStop = stopEvents.filter(e => e.idempotency_key && e.idempotency_key.includes('test-ik-reject-audit'));
...
test('dispatch is blocked during starting and failed phases', async () => {
    ...
    assert.equal(agentLifecycleAllowsDispatch(await readAgentLifecycleState({ dir: f.lifecycleDir, agentId: 'gemini' })), false, 'starting blocks dispatch');
    // Set to failed phase
    await updateAgentLifecycleState({
        dir: f.lifecycleDir, agentId: 'gemini',
        patch: { phase: 'failed', last_error: 'convergence_timeout' },
    });
    assert.equal(agentLifecycleAllowsDispatch(await readAgentLifecycleState({ dir: f.lifecycleDir, agentId: 'gemini' })), false, 'failed blocks dispatch');
    // After successful start: phase=running
    await updateAgentLifecycleState({
        dir: f.lifecycleDir, agentId: 'gemini',
        patch: { phase: 'running', desired_state: 'running', last_error: null },
    });
    assert.equal(agentLifecycleAllowsDispatch(await readAgentLifecycleState({ dir: f.lifecycleDir, agentId: 'gemini' })), true, 'running allows dispatch');
});
...
//# sourceMappingURL=agent-lifecycle-start.test.js.map