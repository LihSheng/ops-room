...
import { ensureTaskWorkspace, selectTaskWorkspacePlan, serializeTaskWorkspace, taskWorkspacePatch, } from '../src/services/task-workspace-binding.js';
...
test('review tasks require and select detached exact-SHA workspaces', () => {
    ...
    assert.throws(() => selectTaskWorkspacePlan(reviewTask({ reviewed_sha: 'main' })), /task_workspace_exact_sha_required/);
});
...
test('task/workspace ownership mismatch fails closed', async () => {
    ...
    await assert.rejects(ensureTaskWorkspace({
...
//# sourceMappingURL=task-workspace-binding.test.js.map