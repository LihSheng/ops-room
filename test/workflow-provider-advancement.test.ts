...
import {
  advanceWorkflowRunWithProviders,
  reconcileReviewDecisionFromEffects,
} from '../src/services/workflow-provider-advancement.js';
...
async function createRun(workflowRunsDir: string, requestKey: string) {
...
function outputForStage(stage: string, inputSha: string) {
  if (stage === 'implementation') return IMPLEMENTATION_SHA;
  if (stage === 'test') return TEST_SHA;
  if (stage === 'integration') return INTEGRATION_SHA;
  return inputSha;
}

function workspaceFor(child: any) {
  return {
...
test('provider-backed coordinator advances all stages and resolves Berlin evidence from the effect store', async () => {
  ...
  const calls: Record<string, number> = { implementation: 0, test: 0, integration: 0, review: 0 };

  const providerAdapters = {
    professor: async ({ child }: any) => {
      calls[child.stage] += 1;
      return { outcome: 'completed', output_sha: outputForStage(child.stage, child.input_sha) };
    },
    tokyo: async ({ child }: any) => {
      calls[child.stage] += 1;
      return { outcome: 'completed', output_sha: TEST_SHA };
    },
    berlin: async ({ child }: any) => {
      calls.review += 1;
      return { outcome: 'completed', output_sha: child.input_sha, review_decision: 'approved' };
    },
  };

  const executeWorkflowChildFn = async ({ workflowRunsDir: dir, workflowId, childId, runStage }: any) => {
  ...
  const result = await advanceWorkflowRunWithProviders({
    ...
    effectsDir,
    providerAdapters,
    resolveStageInstruction: async ({ child }: any) => `Complete ${child.stage}.`,
    executeWorkflowChildFn,
  });
  const run = await readWorkflowRun({ dir: workflowRunsDir, workflowId: created.run.workflow_id });
  const review = run.children.find((child: any) => child.stage === 'review');
...
test('restart reconciliation restores a completed Berlin decision from durable effect evidence', async () => {
  ...
  assert.equal(before.children.find((child: any) => child.stage === 'review').review_decision, undefined);
  ...
  assert.equal(result.run.state, 'completed');
  assert.equal(after.children.find((child: any) => child.stage === 'review').review_decision, 'approved');
