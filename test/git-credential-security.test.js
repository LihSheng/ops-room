...
const { buildAskpassScriptPath, cleanupAskpassHelper, buildAgentEnv, maskToken, renderAskpassScript, } = await import('../src/workflows/github-code.js');
...
test('askpass script does not contain token in file body', () => {
    ...
    const scriptPath = buildAskpassScriptPath(ctx);
    try {
        writeFileSync(scriptPath, renderAskpassScript(), platform() !== 'win32' ? { mode: 0o500 } : {});
        const content = readFileSync(scriptPath, 'utf-8');
        assert.equal(content.includes(token), false, 'token must not be written into the askpass script');
...
test.after(() => {
    ...
    try {
        rmSync(testDirs, { recursive: true, force: true });
    }
    catch { }
});
//# sourceMappingURL=git-credential-security.test.js.map