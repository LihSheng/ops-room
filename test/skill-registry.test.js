...
import { ALLOWED_SKILL_PERMISSIONS, SkillManifestValidationError, validateSkillManifest, } from '../src/services/skill-registry/schema.js';
...
function profile(runtimeBackend = 'opencode') {
    return {
        schemaVersion: 2,
        id: 'agent',
        displayName: 'Agent',
        ...
        skills: [{ key: 'sample-skill', version: '1.0.0' }],
        memory: { read: [], write: [] },
...
test('manifest schema rejects unsupported versions, invalid identity, empty fields, unknown values, duplicates, wildcards, paths, and secret fields', () => {
    const cases = [
        ...
        [manifest({ supportedRuntimes: ['opencode', 'opencode'] }), /duplicate supportedRuntimes/],
        ...
        [manifest({ permissions: [] }), /permissions must not be empty/],
        [manifest({ permissions: ['repository.read', 'repository.read'] }), /duplicate permissions/],
    ...
    for (const [value, pattern] of cases) {
        assert.throws(() => validateSkillManifest(value, 'fixture/manifest.json'), pattern);
    }
...
test('loader rejects directory identity mismatch, version mismatch, duplicate key/version, and non-manifest files', async () => {
    const { root, dir } = await fixture();
    try {
        ...
        await mkdir(join(dir, 'version-mismatch', '2.0.0'), { recursive: true });
        await writeFile(join(dir, 'version-mismatch', '2.0.0', 'manifest.json'), JSON.stringify(manifest({
        ...
        await assert.rejects(() => loadSkillManifests(dir), (error) => {
            ...
            assert.match(error.message, /directory version must match manifest version 1\.0\.0/);
...
test('every canonical profile assignment resolves to the exact declared version', async () => {
    ...
    for (const assignment of [
        { key: loadedProfiles.profiles[0].skills[0].key, version: '9.9.9' },
        { key: 'unknown-skill', version: '1.0.0' },
    ]) {
        const unresolved = resolveSkillAssignments({
            profiles: [{ ...loadedProfiles.profiles[0], skills: [assignment] }],
            manifests,
            commandPresence: {},
...
test('compatibility evaluator reports deterministic runtime, multiple command, and credential reasons', () => {
    const result = evaluateSkillCompatibility({
        profile: profile('gemini'),
        manifest: manifest({ requiredCommands: ['git', 'gh'], requiredCredentials: ['github', 'cloud'] }),
...
test('compatibility evaluator distinguishes one missing command from a present credential', () => {
    const result = evaluateSkillCompatibility({
        profile: profile(),
        manifest: manifest(),
        commandPresence: { git: false },
        credentialResolver: () => 'present',
    });
    assert.equal(result.status, 'incompatible');
    assert.deepEqual(result.reasons.map((reason) => reason.code), ['missing_command']);
    assert.deepEqual(result.requirements.credentials, [{ reference: 'github', status: 'present' }]);
});
