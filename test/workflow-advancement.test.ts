...
function createExecutor(reviewDecisions: Array<any>, options: { omitReviewDecision?: boolean } = {}) {
  const calls: string[] = [];
  let writableOutputIndex = 0;
  let reviewIndex = 0;

  const execute = async ({ workflowRunsDir, workflowId, childId }: any) => {
...
test('advances implementation through Berlin approval and completes once', async () => {
  ...
  assert.deepEqual(stored.children.map((child: any) => child.stage), [
    'implementation',
    'test',
    'integration',
    'review',
  ]);
  assert.ok(stored.children.every((child: any) => child.state === 'completed'));
...
test('creates exactly one next iteration after changes requested', async () => {
  ...
  const firstReview = stored.children.find((child: any) => child.iteration === 1 && child.stage === 'review');
  const secondImplementation = stored.children.find((child: any) => child.iteration === 2 && child.stage === 'implementation');
  const secondReview = stored.children.find((child: any) => child.iteration === 2 && child.stage === 'review');
