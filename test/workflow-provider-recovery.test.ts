...
import {
  reconcileProviderBackedWorkflowRuns,
  resumePendingWorkflowAfterInvestigation,
  retryWorkflowChildAfterInvestigation,
} from '../src/services/workflow-provider-recovery.js';
import {
  readWorkflowRun,
  validateWorkflowRun,
...
} from '../src/services/workspace-store.js';
...
async function writeRun(workflowRunsDir: string, record: any) {
  await import('node:fs/promises').then(({ mkdir }) => mkdir(workflowRunsDir, { recursive: true }));
  await writeFile(
    join(workflowRunsDir, workflowFilename(record.workflow_id)),
    `${JSON.stringify(validateWorkflowRun(record), null, 2)}\n`,
    'utf8',
  );
}

async function writeWorkspace({ recordRoot, state = 'active' }: any) {
...
async function writeEffect({ effectsDir, state, resultCode, outputSha = null }: any) {
...
test('a pending child blocked before provider invocation resumes without incrementing its attempt', async () => {
  ...
  const raw = await readFile(
    join(paths.workflowRunsDir, workflowFilename(WORKFLOW_ID)),
    'utf8',
  );
