...
import { activateWorkflowChild, buildWorkflowChildId, completeWorkflowChild, createOrLoadWorkflowRun, ensureWorkflowChild, failWorkflowChild, readWorkflowRun, retryWorkflowChild, serializeWorkflowRun, validateWorkflowRun, } from '../src/services/workflow-run-store.js';
...
test('parent workflow creation is deterministic and restart-idempotent', async () => {
    ...
    await assert.rejects(createRun(dir, { sourceSha: SHA_B }), /workflow_run_conflict/);
});
test('children enforce canonical ownership, stage order, dependencies, and immutable SHA handoffs', async () => {
    ...
    await assert.rejects(ensureWorkflowChild({
        dir,
        workflowId,
        iteration: 1,
        stage: 'test',
...
test('failed child attempts are recoverable while completed children are immutable', async () => {
    ...
    await assert.rejects(completeWorkflowChild({
        dir,
        workflowId,
        childId: testChild.child.child_id,
        outputSha: SHA_D,
...
//# sourceMappingURL=workflow-run-store.test.js.map