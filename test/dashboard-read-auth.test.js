...
const { sendJSON, verifyAuth, verifyDashboardAuth, verifyDashboardReadRequest, verifyOperatorAuth, } = await import('../src/routes/helpers.js');
...
function request(route, { token = '', cookie = '', method = 'GET' } = {}) {
    return {
        method,
        url: route,
        headers: {
...
function captureJsonResponse(payload = { ok: true }) {
    let statusCode = 0;
    let body = '';
    const res = {
        writeHead(status) {
...
test('asynchronous dashboard authorization preserves the dedicated bearer path', async () => {
    ...
    if (accepted.ok)
        assert.equal(accepted.auth_method, 'dashboard_token');
...
test('sendJSON is a pure response writer after pre-route dashboard authorization', () => {
    assert.deepEqual(captureJsonResponse({ data: 'visible' }), { statusCode: 200, body: { data: 'visible' } });
});
...
//# sourceMappingURL=dashboard-read-auth.test.js.map