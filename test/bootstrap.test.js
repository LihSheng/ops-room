...
import { loadBootstrapEnvironment, main, missingStartupVars, runtimeDirectories, STARTUP_REQUIRED_VARS, } from '../scripts/bootstrap.js';
test('bootstrap loads the selected env file without exposing repo .env', async () => {
    ...
    try {
        ...
        if (previousSecret === undefined)
            ...
            process.env.OPENAB_WEBHOOK_SECRET = previousSecret;
        if (previousPort === undefined)
            delete process.env.OPENAB_WEBHOOK_PORT;
        else
            process.env.OPENAB_WEBHOOK_PORT = previousPort;
