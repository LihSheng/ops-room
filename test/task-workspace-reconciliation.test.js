...
test('batch reconciliation never allocates or destroys workspaces', async () => {
    const results = await reconcileTaskWorkspaces({
        ...
        workspaceRoot: '/workspaces',
        recordRoot: '/records',
        reconcile: async ({ task: value }) => ({ status: 'ready', reason_code: null, workspace: { task_id: value.id } }),
    });
    assert.deepEqual(results.map((item) => item.task_id), ['review-ops-room-32-berlin', 'task-2']);
});
//# sourceMappingURL=task-workspace-reconciliation.test.js.map