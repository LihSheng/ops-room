...
test('runtime adapter registry rejects unsupported and ambiguous definitions', () => {
    assert.throws(() => prepareAgentRuntimes({ definitions: [definition('alpha', 'unsupported')], adapters: [fakeAdapter()] }), /No runtime adapter supports agent alpha/);
    assert.throws(() => prepareAgentRuntimes({ definitions: [definition('alpha')], adapters: [fakeAdapter({ id: 'one' }), fakeAdapter({ id: 'two' })] }), /Multiple runtime adapters support agent alpha/);
});
