...
test('fix child never prepares or pushes when the reviewed SHA is stale', async () => {
    ...
    await assert.rejects(() => runFixChildWorker({
        task: { id: 'fix:1', repository: 'LihSheng/LinkUp', pr: 1, reviewed_sha: 'a'.repeat(40) },
        deps: d,
        lease: MOCK_LEASE,
        workspace: BINDING,
...
test('fix child requires a durable managed workspace binding', async () => {
    const d = deps();
    await assert.rejects(() => runFixChildWorker({ task: { id: 'fix:1', repository: 'LihSheng/LinkUp', pr: 1, reviewed_sha: 'a'.repeat(40) }, deps: d, lease: MOCK_LEASE }), /fix_workspace_binding_missing/);
});
...
//# sourceMappingURL=fix-worker.test.js.map