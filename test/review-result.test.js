...
import { parseStructuredReview } from '../src/workflows/review-result.js';
...
test('structured review rejects approval with blocking findings', () => {
    assert.throws(() => parseStructuredReview(JSON.stringify({
        summary: 'Contradictory', verdict: 'APPROVE', requires_human: false,
        findings: [{ severity: 'high', file: 'src/a.js', line: 1, title: 'Bug', description: 'Bad', auto_fixable: false }],
    })), /cannot include blocking findings/);
});
test('structured review rejects malformed request-changes result', () => {
    assert.throws(() => parseStructuredReview(JSON.stringify({ summary: 'Missing finding', verdict: 'REQUEST_CHANGES', requires_human: false, findings: [] })), /must include findings/);
});
//# sourceMappingURL=review-result.test.js.map