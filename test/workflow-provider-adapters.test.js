...
import { buildWorkflowProviderEnv, createProfileWorkflowProviderAdapters, readWorkflowWorkspaceRemote, runWorkflowProviderProcess, validateWorkflowProviderGitConfig, validateWorkflowProviderRemote, } from '../src/services/workflow-provider-adapters.js';
...
function gitPreflightStub({ fetchRemote = SAFE_REMOTE, pushRemote = SAFE_REMOTE, configKeys = ['core.repositoryformatversion', 'remote.origin.url', 'remote.origin.fetch'], } = {}) {
...
test('workspace Git preflight rejects credential config and unsafe push URLs', async () => {
    await assert.rejects(readWorkflowWorkspaceRemote({
        cwd: '/workspace',
        execFile: gitPreflightStub({
...
test('profile adapters fail closed for disabled, mismatched, unsupported, or unauthorized profiles', async () => {
    ...
    for (const [profileValue, expected] of cases) {
        let calls = 0;
        const adapters = createProfileWorkflowProviderAdapters({
...
test('unsafe workspace remote blocks provider execution before subprocess creation', async () => {
    ...
    await assert.rejects(adapters.professor(adapterInput()), /workflow_provider_remote_credentials_unsafe/);
    assert.equal(calls, 0);
});
...
test('pre-cancelled subprocess execution never spawns a provider', async () => {
    ...
    controller.abort();
    let spawned = false;
    await assert.rejects(runWorkflowProviderProcess({
        command: 'opencode',
        args: ['run', '-'],
        cwd: '/workspace',
        stdin: 'prompt',
...
test('subprocess cancellation waits for the child close event before settling', async () => {
    ...
    let settled = false;
    const observed = promise.then(() => new Error('unexpected provider success'), (error) => error).finally(() => { settled = true; });
    controller.abort();
    assert.deepEqual(child.kills, ['SIGTERM']);
    await delay(10);
...
//# sourceMappingURL=workflow-provider-adapters.test.js.map