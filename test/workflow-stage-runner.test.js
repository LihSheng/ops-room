...
import { createWorkflowStageRunner, parseProviderResult, } from '../src/services/workflow-stage-runner.js';
...
function child(stage, ownerAgent, overrides = {}) {
    return {
...
function runnerInput(stage, ownerAgent, overrides = {}) {
    return {
        run: run(),
...
test('provider output requires an exact 40-character SHA without truncation', () => {
    assert.deepEqual(parseProviderResult({ outcome: 'completed', output_sha: OUTPUT_SHA }), { outcome: 'completed', output_sha: OUTPUT_SHA });
    assert.throws(() => parseProviderResult({ outcome: 'completed', output_sha: `${OUTPUT_SHA}0` }), /workflow_provider_output_invalid/);
    assert.throws(() => parseProviderResult({ outcome: 'completed', output_sha: OUTPUT_SHA.slice(1) }), /workflow_provider_output_invalid/);
});
...
test('stage ownership mismatch fails before any provider invocation', async () => {
    ...
    await assert.rejects(execute(runnerInput('test', 'professor')), /workflow_provider_stage_owner_mismatch/);
...
test('provider timeout waits for adapter shutdown before recording terminal evidence', async () => {
    ...
    let acknowledgeShutdown = null;
    const execute = createWorkflowStageRunner({
        effectsDir,
        providerTimeoutMs: 1_000,
        providerTerminationGraceMs: 1_000,
        providerAdapters: {
            professor: async ({ signal }) => new Promise((_, reject) => {
...
test('provider that ignores SIGTERM and SIGKILL produces termination_failed through real subprocess wrapper', async () => {
    ...
    let spawnCalls = 0;
    const providerAdapters = {
        professor: async ({ prompt, cwd, signal }) => {
            spawnCalls += 1;
...
//# sourceMappingURL=workflow-stage-runner.test.js.map