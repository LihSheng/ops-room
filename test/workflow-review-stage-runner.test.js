...
import { createReviewAwareWorkflowStageRunner, decodeReviewEffectResultCode, parseReviewProviderResult, } from '../src/services/workflow-review-stage-runner.js';
...
test('review provider output requires bounded approval or changes-requested evidence', () => {
    assert.deepEqual(parseReviewProviderResult({ outcome: 'completed', output_sha: SHA, review_decision: 'approved' }), { outcome: 'completed', output_sha: SHA, review_evidence: { decision: 'approved', reason: null } });
    assert.deepEqual(parseReviewProviderResult({
        outcome: 'completed',
        output_sha: SHA,
    ...
    assert.throws(() => parseReviewProviderResult({ outcome: 'completed', output_sha: SHA }), /workflow_review_decision_invalid/);
});
...
//# sourceMappingURL=workflow-review-stage-runner.test.js.map