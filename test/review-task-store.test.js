...
import { buildReviewTaskId, claimTask, createOrClaimTask, isClaimStale, listReviewTasks, readTask, recoverStaleTask, renewClaim, scanReviewTasks, transitionTask, } from '../src/services/review-task-store.js';
test('review task identity is deterministic and changes when SHA or mode changes', () => {
    ...
    assert.notEqual(buildReviewTaskId(input), buildReviewTaskId({ ...input, headSha: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' }));
...
test('lease epoch fencing rejects transitions with mismatched epoch', async () => {
    ...
    const { releaseClaim } = await import('../src/services/review-task-store.js');
    ...
    await assert.rejects(() => transitionTask({ dir, id: task.id, to: 'QUEUED', reason: 'stale-worker', leaseEpoch: 1 }), /Lease epoch mismatch/);
...
test('terminal tasks cannot transition back to running', async () => {
    ...
    await assert.rejects(() => transitionTask({ dir, id: task.id, to: 'RUNNING', reason: 'invalid' }), /Invalid task transition/);
});
...
//# sourceMappingURL=review-task-store.test.js.map