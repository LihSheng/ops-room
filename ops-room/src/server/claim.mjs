 ...
 *
 * Usage:
 *   node openab-claim.mjs                    # List all open tasks across agents
 *   node openab-claim.mjs <agent>            # Claim next task for agent
 *   node openab-claim.mjs <agent> --all      # List all tasks for agent
 *   node openab-claim.mjs <agent> --done     # Mark current task complete
 */
...
function main() {
  ...
  if (flag === '--done') {
    if (!agentName) {
      console.error('Usage: node openab-claim.mjs <agent> --done');
      process.exit(1);
    }
    ...
    for (const issue of issues) {
      ...
      if (hasPending || hasWip) {
        if (hasPending) removeLabel(issue.number, `openab/${agentName}`);
        if (hasWip) removeLabel(issue.number, `openab/${agentName}/wip`);
  ...
  if (labels) {
    ...
    console.log('\nUsage: node openab-claim.mjs <agent>');
  }
}
