...
export function classifyTaskWorkspaceOutcome(task: any) {
  if (!task || !task.state) return 'none';
  if (SUCCESS_STATES.has(task.state)) return 'cleanup';
  if (INVESTIGATION_STATES.has(task.state)) return 'hold';
  return 'preserve';
}

export async function applyTaskWorkspaceOutcome({
  task,
  recordRoot,
  ...
  if (!workspaceId) return { action: 'none', workspace: null };
  ...
  if (action === 'cleanup') {
    ...
    if (record.state !== 'active') throw new Error('task_workspace_cleanup_not_safe');
