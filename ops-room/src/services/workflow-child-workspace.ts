...
function workflowDigest(workflowId: string, length = 16) {
...
function workspaceIdForChild(child: any) {
...
function canonicalFeatureBranch(run: any, child: any) {
  return `agent/professor/feature-${workflowDigest(run.workflow_id)}-i${child.iteration}`;
}

function testBranch(run: any, child: any) {
...
export function selectWorkflowChildWorkspacePlan({ run, child }: any) {
  ...
  if (!SAFE_SHA.test(revision)) throw new Error('workflow_child_workspace_exact_sha_required');

  let mode: 'branch' | 'detached';
...
function taskForChild(run: any, child: any, plan: any, existingWorkspace: any = null) {
...
function validateWorkspaceRecord({ run, child, plan, record }: any) {
  if (!record || typeof record !== 'object') throw new Error('workflow_child_workspace_record_unavailable');
  if (record.workspace_id !== plan.workspace_id) throw new Error('workflow_child_workspace_id_mismatch');
  ...
  if (record.mode !== plan.mode) throw new Error('workflow_child_workspace_mode_mismatch');
  if ((record.branch || null) !== plan.branch) throw new Error('workflow_child_workspace_branch_mismatch');
  ...
  if (record.state !== USABLE_WORKSPACE_STATE) throw new Error('workflow_child_workspace_not_active');

  return serializeTaskWorkspace(record);
}

function sameWorkspace(left: any, right: any) {
  return JSON.stringify(left || null) === JSON.stringify(right || null);
}

async function persistWorkflowChildWorkspace({
  workflowRunsDir,
  workflowId,
  childId,
  workspace,
  ...
  return withWorkspaceLock({
    ...
    execute: async () => {
      ...
      if (current.workspace) {
        if (!sameWorkspace(current.workspace, workspace)) throw new Error('workflow_child_workspace_conflict');
...
export async function ensureWorkflowChildWorkspace({
  workflowRunsDir,
  workflowId,
  childId,
  cacheRoot,
  ...
  try {
    ...
    if (initialChild.workspace || error?.message !== 'workspace_id_conflict') throw error;
