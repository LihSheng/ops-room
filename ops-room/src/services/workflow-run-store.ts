...
function validateRepositoryId(value) {
  const normalized = String(value || '');
  if (!SAFE_REPOSITORY_ID.test(normalized)) throw new Error('invalid_workflow_repository_id');
...
function validateSha(value, field) {
  ...
  if (!SAFE_SHA.test(normalized)) throw new Error(`invalid_${field}`);
  return normalized;
}
...
async function withWorkflowMutation(workflowId, operation) {
  ...
  try {
    ...
    release();
    if (mutationTails.get(workflowId) === tail) mutationTails.delete(workflowId);
  }
}
...
function validateChildBasics(record, child) {
  ...
  if (!FEATURE_DEVELOPMENT_STAGES.includes(child.stage)) throw new Error('invalid_workflow_child_stage');
  ...
  if (!CHILD_STATES.has(child.state)) throw new Error('invalid_workflow_child_state');
  ...
  if (child.child_id !== expectedId) throw new Error('workflow_child_id_mismatch');
  const expectedDependency = expectedDependencyId(record.workflow_id, child.iteration, child.stage);
  if ((child.depends_on || null) !== expectedDependency) throw new Error('workflow_child_dependency_mismatch');
  if (child.state === 'completed') {
    validateSha(child.output_sha, 'workflow_child_output_sha');
    if (!child.completed_at) throw new Error('workflow_child_completion_evidence_required');
...
export function buildWorkflowRunId({ repository, requestKey }) {
  ...
  if (!key || key.length > 500) throw new Error('invalid_workflow_request_key');
...
export function buildWorkflowChildId({ workflowId, iteration, stage }) {
  ...
  if (!FEATURE_DEVELOPMENT_STAGES.includes(stage)) throw new Error('invalid_workflow_child_stage');
  return `${workflowId}:${iteration}:${stage}`;
}

export function validateWorkflowRun(record) {
  if (!record || typeof record !== 'object' || Array.isArray(record)) throw new Error('invalid_workflow_run');
  ...
  if (record.workflow_type !== FEATURE_DEVELOPMENT_WORKFLOW) throw new Error('unsupported_workflow_type');
  ...
  if (!WORKFLOW_STATES.has(record.state)) throw new Error('invalid_workflow_state');
  ...
  if (!Array.isArray(record.children)) throw new Error('invalid_workflow_children');
  if (!Array.isArray(record.history)) throw new Error('invalid_workflow_history');

  const byId = new Map();
  const byStageIteration = new Map();
  for (const child of record.children) {
    validateChildBasics(record, child);
    if (byId.has(child.child_id)) throw new Error('duplicate_workflow_child_id');
    const stageKey = `${child.iteration}:${child.stage}`;
    if (byStageIteration.has(stageKey)) throw new Error('duplicate_workflow_child_stage');
  ...
  for (const child of record.children) {
    ...
    if (!dependency) throw new Error('workflow_child_dependency_unavailable');
    if (dependency.state !== 'completed' || !dependency.output_sha) {
      throw new Error('workflow_child_dependency_incomplete');
    }
    if (child.input_sha !== dependency.output_sha) throw new Error('workflow_child_input_mismatch');
  }
  return record;
}
...
export async function createOrLoadWorkflowRun({ dir, input, policy = {}, now = nowIso }) {
  ...
  try {
    ...
    if (
      existing.repository_id !== repositoryId
...
export async function ensureWorkflowChild({ dir, workflowId, iteration, stage, inputSha, now = nowIso }) {
  return withWorkflowMutation(workflowId, async () => {
    ...
    if (!dependencyId) {
      if (normalizedInputSha !== run.source_sha) throw new Error('workflow_child_root_input_mismatch');
    } else {
      ...
      if (dependency.output_sha !== normalizedInputSha) throw new Error('workflow_child_input_mismatch');
    }
...
export async function activateWorkflowChild({ dir, workflowId, childId, now = nowIso }) {
  return withWorkflowMutation(workflowId, async () => {
    ...
    if (current.state === 'active') return { run, child: current };
    if (current.state !== 'pending') throw new Error(`workflow_child_not_activatable:${current.state}`);
    const activeCount = run.children.filter((child) => child.state === 'active').length;
    if (activeCount >= run.policy.max_concurrency) throw new Error('workflow_concurrency_limit');
...
export async function completeWorkflowChild({ dir, workflowId, childId, outputSha, now = nowIso }) {
  return withWorkflowMutation(workflowId, async () => {
    ...
    if (current.state === 'completed') {
      if (current.output_sha !== normalizedOutputSha) throw new Error('workflow_child_completion_conflict');
      return { run, child: current };
    }
    if (current.state !== 'active') throw new Error(`workflow_child_not_completable:${current.state}`);
    const at = now();
    const child = {
      ...current,
      state: 'completed',
...
export async function failWorkflowChild({ dir, workflowId, childId, error, now = nowIso }) {
  return withWorkflowMutation(workflowId, async () => {
    ...
    if (current.state === 'failed' && current.last_error === boundedError) return { run, child: current };
    if (current.state !== 'active') throw new Error(`workflow_child_not_failable:${current.state}`);
    const at = now();
    const child = {
      ...current,
      state: 'failed',
...
export async function retryWorkflowChild({ dir, workflowId, childId, now = nowIso }) {
  return withWorkflowMutation(workflowId, async () => {
    ...
    if (index < 0) throw new Error('workflow_child_not_found');
    const current = run.children[index];
    if (current.state !== 'failed') throw new Error(`workflow_child_not_retryable:${current.state}`);
    const at = now();
    const child = {
      ...current,
      state: 'pending',
