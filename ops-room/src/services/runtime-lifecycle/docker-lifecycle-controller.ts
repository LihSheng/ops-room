...
function runBoundedCommand(command: string, args: string[], { timeoutMs }: { timeoutMs: number }) {
  return new Promise<void>((resolve, reject) => {
    ...
    child.once('close', (code) => {
      if (code === 0) finish();
      else finish(new Error('runtime command failed'));
    });
  });
}
  ...
  return {
    ...
    async stop(preparedRuntime, { timeoutSeconds = 20 } = {}) {
      ...
      try {
        await runCommand(
          'docker',
          ['stop', '--time', String(timeout), containerName],
