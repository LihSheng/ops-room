...
function boundedChild(child: any) {
  return {
...
function validateExecutionOutcome(value: any) {
  ...
  if (!SAFE_SHA.test(outputSha)) throw new Error('workflow_child_output_sha_invalid');
  return {
...
function validateWorkspaceOwnership({ run, child, record }: any) {
  ...
  if (!workspaceId) throw new Error('workflow_child_workspace_binding_missing');
  if (record.workspace_id !== workspaceId) throw new Error('workflow_child_workspace_id_mismatch');
...
async function markWorkflowChildNeedsHuman({
  workflowRunsDir,
  workflowId,
  childId,
  reason,
  ...
  if (current.state === 'completed') throw new Error('workflow_child_completion_immutable');
...
async function applyWorkflowWorkspaceOutcome({
  run,
  child,
  recordRoot,
  outcome,
  ...
  if (outcome === 'completed') {
    ...
    if (record.state !== 'active') throw new Error('workflow_child_workspace_cleanup_not_safe');
...
export async function executeWorkflowChild({
  ...
  activateChild = activateWorkflowChild,
  completeChild = completeWorkflowChild,
  markNeedsHuman = markWorkflowChildNeedsHuman,
  applyWorkspaceOutcome = applyWorkflowWorkspaceOutcome,
  executionLockDir = join(workflowRunsDir, '.locks'),
  executionLockTimeoutMs = 10_000,
  executionLockStaleAfterMs = DEFAULT_EXECUTION_LOCK_STALE_AFTER_MS,
  now = () => new Date().toISOString(),
}: any) {
  if (typeof runStage !== 'function') throw new Error('workflow_child_stage_runner_required');
  ...
  return withWorkspaceLock({
    ...
    execute: async () => {
      ...
      if (child.state !== 'pending') throw new Error(`workflow_child_not_executable:${child.state}`);
      ...
      try {
        ...
        if (outcome.outcome === 'completed') {
          if (child.stage === 'review') {
            ...
            if (!SAFE_SHA.test(actualSha)) throw new Error('workflow_workspace_head_invalid');
            if (actualSha !== outcome.output_sha) throw new Error('workflow_child_output_sha_mismatch');
          }
        }
      } catch (error: any) {
