...
function eventPath(dir, eventId) {
  if (!SAFE_EVENT_ID.test(String(eventId))) throw new Error('Invalid audit event ID');
...
export async function listAuditEvents({
  ...
  for (const name of names.filter((entry) => /^event-[A-Fa-f0-9-]+\.json$/.test(entry))) {
    try {
      ...
      if (from && String(event.created_at) < String(from)) continue;
      if (to && String(event.created_at) > String(to)) continue;
      events.push(event);
    } catch (error) {
