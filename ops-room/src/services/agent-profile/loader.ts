...
export async function loadAgentProfiles(dir: string): Promise<LoadedAgentProfiles> {
  ...
  for (const profile of profiles) {
    if (seen.has(profile.id)) issues.push(`duplicate profile id: ${profile.id}`);
    seen.add(profile.id);
  }
  ...
  for (const definition of AGENT_DEFINITIONS) {
    if (!seen.has(definition.key)) issues.push(`${definition.key}: missing required profile`);
  }
