...
test('fenced: stale worker rejected from new effect when task has different lease', async () => {
  ...
  await assert.rejects(
    () => claimEffect({ dir, taskId, kind: 'github_review', fingerprint: 'sha:fenced-new', leaseId: leaseA.lease_id, leaseEpoch: leaseA.lease_epoch }),
    /Stale lease/,
    'Stale worker A should be rejected when task is owned by B',
  );
});
...
test('fenced: stale worker rejected when task has no current lease (retry window)', async () => {
  ...
  await assert.rejects(
    () => claimEffect({ dir, taskId, kind: 'github_review', fingerprint: 'sha:null-lease-new', leaseId: leaseA.lease_id, leaseEpoch: leaseA.lease_epoch }),
    /no current lease/,
    'Stale worker should be rejected when task has no current lease',
  );
});
...
test('fenced: stale worker cannot complete effect after task lease changed', async () => {
  ...
  await assert.rejects(
    () => completeEffect({ dir, effectId: effect.effect.id, result: { event: 'APPROVE' }, leaseId: leaseA.lease_id, leaseEpoch: leaseA.lease_epoch }),
    /Stale lease/,
    'Stale worker should be rejected from completing effect after lease changed',
  );
});
