...
export async function reconcileTaskWorkspace({ task, workspaceRoot, recordRoot, readRecord = readWorkspaceRecord, statPath = stat, }) {
    ...
    if (record.task_id !== task.id)
        return { status: 'blocked', reason_code: 'workspace_task_mismatch', workspace: null };
    if (record.owner_agent !== task.agent)
        return { status: 'blocked', reason_code: 'workspace_owner_mismatch', workspace: null };
    if (record.repository_id !== task.repository)
        return { status: 'blocked', reason_code: 'workspace_repository_mismatch', workspace: null };
    let workspacePath;
    try {
...
export async function reconcileTaskWorkspaces({ tasks, workspaceRoot, recordRoot, reconcile = reconcileTaskWorkspace, }) {
    const results = [];
...
//# sourceMappingURL=task-workspace-reconciliation.js.map