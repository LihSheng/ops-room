...
import { claimWorkflowEffect, completeWorkflowEffect, } from './workflow-effect-store.js';
...
function boundedReason(value, fallback = 'workflow_provider_failed', max = 120) {
...
function normalizeReviewReason(value, decision) {
    if (decision === 'approved')
        return null;
    if (value == null || String(value).trim() === '')
        return 'workflow_review_changes_requested';
    return boundedReason(value, 'workflow_review_changes_requested', MAX_REVIEW_REASON_LENGTH);
}
function parseProviderObject(value) {
    let parsed = value;
...
export function parseReviewProviderResult(value) {
...
export function encodeReviewEffectResultCode(evidence) {
...
export function decodeReviewEffectResultCode(value) {
    ...
    if (!reason || !SAFE_REASON.test(reason))
        return null;
    return { decision: 'changes_requested', reason };
}
export function renderReviewStagePrompt({ run, child, instruction }) {
    return [
...
async function invokeWithDeadline({ invoke, timeoutMs, terminationGraceMs, signal }) {
    if (signal?.aborted)
        throw new Error('workflow_provider_cancelled');
    ...
    const provider = Promise.resolve()
        ...
        .then((value) => ({ source: 'provider', status: 'resolved', value }), (error) => ({ source: 'provider', status: 'rejected', error }));
...
function replayReviewEffect(effect) {
    if (effect.state === 'completed' && SAFE_SHA.test(String(effect.output_sha || ''))) {
        ...
        if (!reviewEvidence) {
            return { outcome: 'needs_human', reason: 'workflow_review_effect_evidence_invalid' };
        }
        return {
            outcome: 'completed',
    ...
    if (effect.state === 'claimed') {
        return { outcome: 'needs_human', reason: 'workflow_effect_interrupted' };
    }
    return {
        outcome: 'needs_human',
...
export function createReviewAwareWorkflowStageRunner({ effectsDir, providerAdapters, resolveStageInstruction, providerTimeoutMs = DEFAULT_PROVIDER_TIMEOUT_MS, providerTerminationGraceMs = DEFAULT_PROVIDER_TERMINATION_GRACE_MS, signal = null, claimEffect = claimWorkflowEffect, completeEffect = completeWorkflowEffect, }) {
    ...
    const boundedTerminationGraceMs = Math.max(100, Math.min(Number(providerTerminationGraceMs) || DEFAULT_PROVIDER_TERMINATION_GRACE_MS, 60_000));
    return async function runWorkflowStage(input) {
        const { run, child, workspace_path, workspace } = input;
        if (child?.stage !== 'review')
            return baseRunner(input);
        if (!effectsDir)
            throw new Error('workflow_effects_dir_required');
        ...
        const instruction = boundedText(await resolveStageInstruction({ run, child }), MAX_STAGE_INSTRUCTION_LENGTH);
        if (!instruction)
...
//# sourceMappingURL=workflow-review-stage-runner.js.map