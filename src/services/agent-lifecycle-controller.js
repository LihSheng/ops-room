...
import { classifyConvergence, isApprovedHealth, readAgentLifecycleState, updateAgentLifecycleState, withAgentLifecycleGate, } from './agent-lifecycle-store.js';
import { executeIdempotent, IdempotencyConflictError, IdempotencyInProgressError, validateIdempotencyKey, } from './idempotency-store.js';
...
export async function withLifecycleActionLock(execute) {
...
function observedRuntime(snapshot, agentId) {
    const instance = (snapshot?.instances || []).find((entry) => (entry?.agent === agentId || entry?.agent_id === agentId || entry?.definition?.key === agentId));
...
function inspectObservedRuntime(getRuntimeSnapshot, agentId, fallbackAdapterId = null) {
    try {
        ...
        return { adapter_id: fallbackAdapterId, status: 'unknown', health: 'unknown', state: 'unknown' };
    }
}
// ── drain ──────────────────────────────────────────────────
async function waitForAgentDrain({ reviewTasksDir, agentId, timeoutMs, pollMs, scanTasks, now, sleep, }) {
    const startedAt = now();
    ...
    while (true) {
        ...
        const active = (scanned.tasks || []).filter((task) => (task.agent === agentId && ACTIVE_TASK_STATES.has(task.state)));
        if (initialActive === null)
            initialActive = active.length;
        if (active.length === 0) {
            return {
                drained: true,
...
function confirmationFrom(body, agentId) {
    ...
    if (confirmation !== agentId)
        throw new Error('confirm_agent_id must exactly match the target agent');
    return confirmation;
}
...
async function rejected({ auditDir, actor, agentId, reason, idempotencyKey, errorCode, status, message, previousState = null, resultingState = previousState, metadata = {}, operation = 'agent.stop', }) {
...
// ── stop ───────────────────────────────────────────────────
export async function stopAgent({ agentId, body, actor, reviewTasksDir, lifecycleDir, auditDir, idempotencyDir, allowedAgents = [], drainTimeoutMs = 30_000, drainPollMs = 500, stopTimeoutSeconds = 30, getRuntimeSnapshot, prepareTarget, scanTasks, now = () => Date.now(), nowIso = () => new Date().toISOString(), sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms)), }) {
    ...
    try {
        const result = await executeIdempotent({
            ...
            execute: () => withLifecycleActionLock(() => withAgentLifecycleGate(rawAgentId, async () => {
                ...
                const observed = inspectObservedRuntime(getRuntimeSnapshot, rawAgentId, target.runtime_adapter_id || null);
...
// ── start ──────────────────────────────────────────────────
export async function startAgent({ agentId, body, actor, reviewTasksDir, lifecycleDir, auditDir, idempotencyDir, allowedAgents = [], startTimeoutSeconds = 30, getRuntimeSnapshot, freshRuntimeSnapshot, prepareTarget, scanTasks, now = () => Date.now(), nowIso = () => new Date().toISOString(), sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms)), }) {
    const rawAgentId = String(agentId || '');
    let reason = String(body?.reason || '').trim().slice(0, 500);
    let idempotencyKey = body?.idempotency_key ? String(body.idempotency_key).trim() : null;
    if (!SAFE_AGENT_ID.test(rawAgentId)) {
...
//# sourceMappingURL=agent-lifecycle-controller.js.map