...
function manifestId(key, version) {
    return `${key}@${version}`;
}
...
export async function loadSkillManifests(dir) {
    const issues = [];
    const manifests = [];
    const sources = {};
    const seen = new Map();
    ...
    for (const skillEntry of skillEntries) {
        ...
        for (const versionEntry of versionEntries) {
            ...
            try {
                ...
                if (manifest.key !== skillEntry.name)
                ...
                else if (error instanceof SkillManifestValidationError)
    ...
    if (manifests.length === 0)
        issues.push(`${dir}: no valid skill manifests found`);
    if (issues.length)
