...
function validateNormalizedArray({
  ...
  if (requireNonEmpty && values.length === 0) issues.push(`${source}: ${field} must not be empty`);
  for (const item of values) {
    if (!item || item !== item.trim() || item !== item.toLowerCase()) {
      issues.push(`${source}: ${field} entries must be normalized lowercase values`);
      continue;
    }
    if (containsUnsafePath(item)) issues.push(`${source}: ${field} contains an unsafe path value`);
    if (pattern && !pattern.test(item)) issues.push(`${source}: ${field} contains invalid value ${item}`);
    if (allowlist && !allowlist.has(item)) issues.push(`${source}: ${field} contains unsupported value ${item}`);
  }
  const duplicates = duplicateValues(values);
  if (duplicates.length) issues.push(`${source}: duplicate ${field}: ${duplicates.join(', ')}`);
  return values;
}

export function isValidSemanticVersion(value: string) {
  return SEMANTIC_VERSION_PATTERN.test(value);
}

export function validateSkillManifest(value: unknown, source: string): SkillManifest {
  if (!isRecord(value)) {
