...
function sortReasons(reasons) {
    return reasons.sort((left, right) => (REASON_ORDER[left.code] - REASON_ORDER[right.code] ||
        String(left.subject || '').localeCompare(String(right.subject || ''))));
}
export function evaluateSkillCompatibility({ profile, manifest, commandPresence, credentialResolver, }) {
    if (!manifest) {
        return {
            status: 'unknown',
    ...
    const commands = manifest.requiredCommands.map((name) => {
        ...
        if (!present)
            reasons.push({ code: 'missing_command', subject: name, message: `Required command ${name} was not detected.` });
        return { name, status: present ? 'present' : 'missing' };
    });
