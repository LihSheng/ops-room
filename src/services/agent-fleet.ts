...
export type AgentFleetState = typeof AGENT_FLEET_STATES[number];
...
function bounded(value: unknown, maximum = 200): string | null {
  if (value == null) return null;
  const normalized = String(value).trim();
  return normalized ? normalized.slice(0, maximum) : null;
}

function normalizeTaskState(task: any): string {
...
function taskTimestamp(task: any): string | null {
...
function workspaceSummary(task: any) {
...
function selectCurrentTask(tasks: any[]) {
  return tasks
    .slice()
...
function attentionFor({ profile, runtimeAgent, currentTask }: any) {
  if (!profile) {
    return {
      required: true,
...
function fleetState({ profile, runtimeAgent, currentTask, attention }: any): AgentFleetState {
  if (!profile || !profile.enabled) return 'unavailable';

  const attentionReason = String(attention?.reason_code || '');
  if (attention?.required) {
    if (attentionReason === 'runtime_unavailable') return 'unavailable';
    return 'needs_human';
  }

  const taskState = normalizeTaskState(currentTask);
  if (currentTask && PAUSED_TASK_STATES.has(taskState)) return 'paused';
  if (currentTask && WORKING_TASK_STATES.has(taskState)) return 'working';
  if (currentTask && WAITING_TASK_STATES.has(taskState)) return 'waiting';

  if (runtimeAgent?.convergence_status === 'transitioning') return 'waiting';
  const runtimeStatus = String(runtimeAgent?.observed_state || runtimeAgent?.runtime?.status || 'unknown').toLowerCase();
  if (AVAILABLE_RUNTIME_STATES.has(runtimeStatus)) return 'idle';
  if (OFFLINE_RUNTIME_STATES.has(runtimeStatus)) return 'offline';
  return 'unavailable';
}

export function buildAgentFleet({ agents = [], profiles = [], tasks = [], tasksAvailable = true, generatedAt = new Date().toISOString() }: any = {}) {
  ...
  const fleet = [...allIds]
    ...
    .map((id) => {
      const profile: any = profileById.get(id) || null;
      const runtimeAgent: any = runtimeById.get(id) || null;
...
export async function getAgentFleet({
  ...
  getProfiles = listAgentProfiles,
  getTasks = readTasksDir,
