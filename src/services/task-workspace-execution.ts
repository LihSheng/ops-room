...
function activeStateForTask(task: any) {
  return task?.kind === 'fix' ? 'FIXING' : 'RUNNING';
}

function validateTerminalState(state: string) {
...
export async function executeTaskInWorkspace({
  task,
  taskDir,
  cacheRoot,
  workspaceRoot,
  ...
  if (!task?.id || typeof execute !== 'function') throw new Error('task_workspace_execution_input_invalid');
