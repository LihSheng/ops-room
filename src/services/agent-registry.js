...
export async function getAgentList({ getRuntimeSnapshot = inspectAgentRuntimes, getLifecycleState = readAgentLifecycleState, lifecycleDir = LIFECYCLE_DIR, } = {}) {
    const agents = [];
    const runtimeSnapshot = getRuntimeSnapshot();
    const runtimeByAgent = new Map((runtimeSnapshot.instances || []).map((instance) => [instanceAgentId(instance), instance]));
    for (const definition of AGENT_DEFINITIONS) {
        ...
        if (!hasExample)
            missing.push(exampleConfigPath);
        const inspected = runtimeByAgent.get(key);
        const runtime = inspected?.runtime || null;
        const convergence = lifecycle ? classifyConvergence(lifecycle.desired_state || definition.desiredState, lifecycle.phase, runtime?.status || 'unknown') : { status: 'unknown', reason_code: 'observed_unknown' };
        agents.push({
            key,
