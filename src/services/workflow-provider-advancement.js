...
import { advanceWorkflowRun, persistReviewDecision, } from './workflow-advancement.js';
...
import { createReviewAwareWorkflowStageRunner, decodeReviewEffectResultCode, } from './workflow-review-stage-runner.js';
...
function latestUnresolvedCompletedReview(run) {
    return [...run.children]
...
export async function resolveReviewDecisionFromEffects({ effectsDir, workflowId, child }) {
    if (!effectsDir || !workflowId || !child?.child_id)
        return null;
    ...
    const matches = effects.filter((effect) => (effect.effect_type === 'provider.berlin.review'
...
export async function reconcileReviewDecisionFromEffects({ workflowRunsDir, workflowId, effectsDir, readRun = readWorkflowRun, persistDecision = persistReviewDecision, lockDir = join(workflowRunsDir, '.review-decision-locks'), lockTimeoutMs = 10_000, lockStaleAfterMs = DEFAULT_REVIEW_RECONCILIATION_LOCK_STALE_AFTER_MS, now = () => new Date().toISOString(), }) {
    return withWorkspaceLock({
        ...
        execute: async () => {
            ...
            if (!child)
                return { reconciled: false, reason: 'workflow_review_decision_not_pending' };
            const evidence = await resolveReviewDecisionFromEffects({ effectsDir, workflowId, child });
            if (!evidence)
                return { reconciled: false, reason: 'workflow_review_decision_evidence_unavailable', child };
...
export async function advanceWorkflowRunWithProviders({ workflowRunsDir, workflowId, effectsDir, resolveStageInstruction, providerAdapters = createProfileWorkflowProviderAdapters(), providerTimeoutMs, providerTerminationGraceMs, signal, childExecutionOptions = {}, advancementOptions = {}, executeWorkflowChildFn = executeWorkflowChild, advanceWorkflowRunFn = advanceWorkflowRun, }) {
    if (!workflowRunsDir || !workflowId || !effectsDir)
        throw new Error('workflow_provider_advancement_input_invalid');
    ...
    return advanceWorkflowRunFn({
        ...
        workflowRunsDir,
        workflowId,
        executeChild: ({ childId }) => executeWorkflowChildFn({
        ...
        resolveReviewDecision: ({ child }) => resolveReviewDecisionFromEffects({
            effectsDir,
            workflowId,
            child,
        }),
...
//# sourceMappingURL=workflow-provider-advancement.js.map