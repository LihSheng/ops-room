...
import { countActiveTasks, checkConcurrency, claimTask, transitionTask, recoverStaleTask, scanReviewTasks, } from './review-task-store.js';
...
export async function reconcileReviewTasks({ dir, now, staleMinutes = 30, retryLimit = 3, workspaceRoot = TASK_WORKSPACE_ROOT, workspaceRecordRoot = WORKSPACE_RECORDS_DIR, reconcileWorkspace = reconcileTaskWorkspace, applyWorkspaceOutcome = applyTaskWorkspaceOutcome, }) {
    ...
    for (const task of tasks) {
        ...
        if (workspace.status === 'legacy_unbound')
            legacyUnbound.push(task.id);
        const result = await recoverStaleTask({ dir, id: task.id, now, staleMinutes, retryLimit });
        if (result.recovered) {
            recovered.push(task.id);
            if (result.re_dispatched)
                reDispatched.push(task.id);
        }
    }
    return {
        scanned,
...
async function releaseSlotLock(result) {
    if (result?.paths)
        await releaseSlotLocks(result.paths);
}
...
export async function dispatchEligibleTasks({ dir, instanceId, canDispatchAgent = async () => true, withAgentDispatchGate = withAgentLifecycleGate, }) {
    const { tasks } = await scanReviewTasks({ dir });
    const dispatched = [];
    for (const task of tasks) {
        if (!DISPATCHABLE_STATES.has(task.state))
            continue;
        const claimedTask = await withAgentDispatchGate(task.agent, async () => {
            if (!(await dispatchAllowed(canDispatchAgent, task.agent)))
                return null;
        ...
        if (claimedTask)
            dispatched.push(claimedTask);
    }
    return { dispatched: dispatched.length, tasks: dispatched };
}
//# sourceMappingURL=review-reconciler.js.map