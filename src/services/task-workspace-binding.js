...
function implementationBranch(task) {
...
export function selectTaskWorkspacePlan(task) {
    if (!task || !task.id || !task.repository || !task.agent)
        throw new Error('task_workspace_input_invalid');
    ...
    if (reviewLike) {
        if (!SAFE_SHA.test(reviewedSha))
            throw new Error('task_workspace_exact_sha_required');
        return {
...
export function serializeTaskWorkspace(record) {
    if (!record)
        return null;
    return {
...
async function validateExistingBinding({ task, workspaceRoot, recordRoot, workspaceId, readRecord, statPath, }) {
    let record;
    try {
    ...
    if (!USABLE_STATES.has(record.state))
        throw new Error('task_workspace_not_usable');
    ...
    try {
        ...
        if (error?.message === 'task_workspace_missing')
            throw error;
        throw new Error('task_workspace_missing');
    }
    return { record, workspace_path: workspacePath, reused: true };
}
export async function ensureTaskWorkspace({ task, cacheRoot, workspaceRoot, recordRoot, lockRoot, remote, maxActiveWorkspaces, minimumFreeBytes, getFreeBytes, allocate = allocateWorkspace, readRecord = readWorkspaceRecord, statPath = stat, }) {
