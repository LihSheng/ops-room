...
function boundedReason(value, fallback = 'workflow_provider_failed') {
    const normalized = boundedText(value, 120).toLowerCase();
    return SAFE_REASON.test(normalized) ? normalized : fallback;
}
function stageAuthority(child) {
    const authority = STAGE_AUTHORITY[child?.stage];
    if (!authority)
        throw new Error('workflow_provider_stage_invalid');
...
function renderStagePrompt({ run, child, instruction }) {
...
function parseProviderResult(value) {
    ...
    if (!SAFE_SHA.test(outputSha))
        throw new Error('workflow_provider_output_invalid');
    return { outcome: 'completed', output_sha: outputSha };
}
function replayEffect(effect) {
    if (effect.state === 'completed' && SAFE_SHA.test(String(effect.output_sha || ''))) {
        return { outcome: 'completed', output_sha: String(effect.output_sha).toLowerCase() };
    }
...
export function createWorkflowStageRunner({ effectsDir, providerAdapters, resolveStageInstruction, providerTimeoutMs = DEFAULT_PROVIDER_TIMEOUT_MS, providerTerminationGraceMs = DEFAULT_PROVIDER_TERMINATION_GRACE_MS, signal = null, claimEffect = claimWorkflowEffect, completeEffect = completeWorkflowEffect, }) {
    if (!effectsDir)
    ...
    return async function runWorkflowStage({ run, child, workspace_path, workspace }) {
        ...
        if (!claim.execute)
            return replayEffect(claim.effect);
        try {
