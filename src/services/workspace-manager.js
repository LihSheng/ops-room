...
import { assertPathWithinRoot, ensureRepositoryCache, repositoryCacheKey, repositoryCachePath, resolveRepositoryRevision, validateRepositoryId, } from './repository-cache.js';
...
export async function allocateWorkspace({ cacheRoot, workspaceRoot, recordRoot, lockRoot, repositoryId, remote, workspaceId, ownerAgent, taskId, mode, branch = null, revision = 'HEAD', maxActiveWorkspaces = 8, minimumFreeBytes = 0, getFreeBytes = async () => Number.MAX_SAFE_INTEGER, execFile = execFileDefault, now = () => new Date().toISOString(), }) {
    ...
    if (mode === 'branch' && !SAFE_BRANCH.test(String(branch || '')))
        throw new Error('invalid_workspace_branch');
    if (mode === 'detached' && !SAFE_SHA.test(String(revision || '')))
        throw new Error('detached_workspace_requires_exact_sha');
    await Promise.all([
    ...
    return withWorkspaceLock({
        ...
        execute: async () => {
            ...
            if ((await countActive(records)) >= maxActiveWorkspaces)
                throw new Error('workspace_capacity_exceeded');
            if (await getFreeBytes(workspaceRoot) < minimumFreeBytes)
                throw new Error('workspace_low_disk');
            if (records.some((record) => record.workspace_id === id && ACTIVE_STATES.has(record.state))) {
                throw new Error('workspace_id_conflict');
            }
            if (mode === 'branch' && records.some((record) => (record.repository_id === repoId
                && record.mode === 'branch'
...
export async function requestWorkspaceCleanup({ recordRoot, workspaceId, holdReason = null, now }) {
    ...
    if (current.state === 'held_for_investigation')
        throw new Error('workspace_held');
    if (holdReason) {
    ...
    if (current.state !== 'active' && current.state !== 'failed')
        throw new Error('workspace_cleanup_not_allowed');
    return writeWorkspaceRecord({ dir: recordRoot, record: { ...current, state: 'cleanup_requested' }, now });
}
export async function cleanupWorkspace({ cacheRoot, workspaceRoot, recordRoot, lockRoot, workspaceId, execFile = execFileDefault, now = () => new Date().toISOString(), }) {
    return withWorkspaceLock({
        dir: lockRoot,
        ...
        execute: async () => {
            ...
            if (current.state === 'released')
                return current;
            if (current.state !== 'cleanup_requested')
                throw new Error('workspace_cleanup_not_requested');
