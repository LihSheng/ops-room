...
import { TASKS_DIR, REVIEW_TASKS_DIR, MISSIONS_DIR, STATE_DIR, LOCK_DIR, WORKSPACE_BASE, LOG_DIR, PROMPT_DIR, AUDIT_DIR, IDEMPOTENCY_DIR, LIFECYCLE_DIR, } from './runtime-paths.js';
...
export async function releaseLock(ctx) {
    try {
        await rm(lockPath(ctx), { force: true });
    }
    catch { }
}
...
export async function readTaskById(taskId) {
    ...
    if (!/^[A-Za-z0-9._-]+$/.test(safeTaskId))
        return null;
    try {
