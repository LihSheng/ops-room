...
const APPROVED_HEALTH_STATES = new Set(['healthy', 'none']);
...
function validateAgentId(agentId) {
    ...
    if (!SAFE_AGENT_ID.test(value))
        throw new Error(`Invalid agent ID: ${agentId}`);
    return value;
}
function statePath(dir, agentId) {
...
export function unmanagedAgentLifecycleState(agentId) {
    return {
...
function unavailableAgentLifecycleState(agentId) {
    return {
...
function normalizeLastOperation(value) {
    ...
    if (value.operation !== 'agent.stop' && value.operation !== 'agent.start')
        throw new Error('Invalid lifecycle operation');
    if (!OPERATION_OUTCOMES.has(value.outcome))
        throw new Error('Invalid lifecycle operation outcome');
    return {
...
function normalizeLifecycleState(agentId, value) {
...
function buildLifecycleState(agentId, value) {
    const normalizedId = validateAgentId(agentId);
...
export async function readAgentLifecycleState({ dir, agentId }) {
    const normalizedId = validateAgentId(agentId);
    try {
        ...
        if (error?.code === 'ENOENT')
            return unmanagedAgentLifecycleState(normalizedId);
        return unavailableAgentLifecycleState(normalizedId);
    }
}
export function readAgentLifecycleStateSync({ dir, agentId }) {
    const normalizedId = validateAgentId(agentId);
    try {
...
export async function updateAgentLifecycleState({ dir, agentId, patch, now = nowIso, }) {
    const normalizedId = validateAgentId(agentId);
...
export function agentLifecycleAllowsDispatch(state) {
    if (!state || state.last_error === 'lifecycle_state_unavailable')
        return false;
    if (state.desired_state === 'stopped')
        return false;
    return !BLOCKED_DISPATCH_PHASES.has(state.phase);
}
export async function withAgentLifecycleGate(agentId, execute) {
    const normalizedId = validateAgentId(agentId);
...
export async function recoverInterruptedAgentLifecycleStates({ dir, now = nowIso, }) {
    let names;
    try {
    ...
    for (const name of names.filter((entry) => /^agent-[A-Za-z0-9._-]+\.json$/.test(entry))) {
        ...
        if (!INTERRUPTED_PHASES.has(state.phase))
            continue;
...
export function isApprovedHealth(health) {
    if (!health)
        return false;
    return APPROVED_HEALTH_STATES.has(health);
}
export function classifyConvergence(desiredState, phase, observedState, health) {
    ...
    if (desiredState === 'running' && phase === 'running') {
        if (observedState === 'running') {
            if (health && !isApprovedHealth(health)) {
                if (health === 'unhealthy')
                    return { status: 'mismatch', reason_code: 'observed_unhealthy' };
                if (health === 'starting')
                    return { status: 'transitioning', reason_code: 'health_starting' };
                return { status: 'mismatch', reason_code: 'observed_not_healthy' };
            }
            return { status: 'converged', reason_code: null };
        }
    ...
    if (desiredState === 'stopped' && phase === 'stopped') {
        ...
        if (stoppedObserved.has(observedState))
            return { status: 'converged', reason_code: null };
...
//# sourceMappingURL=agent-lifecycle-store.js.map