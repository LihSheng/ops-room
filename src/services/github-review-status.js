import { claimEffect, completeEffect, reclaimEffect } from './review-effect-ledger.js';
...
export function createGitHubReviewStatusService({ getCommitStatuses, createCommitStatus }) {
    if (typeof getCommitStatuses !== 'function')
        throw new Error('getCommitStatuses is required');
    if (typeof createCommitStatus !== 'function')
        throw new Error('createCommitStatus is required');
    async function set({ repository, sha, state, description, targetUrl, agent = 'professor', dir, taskId, leaseId, leaseEpoch }) {
        ...
        if (effect?.effect)
            await completeEffect({ dir, effectId: effect.effect.id, result: { sha, state, description, context: REVIEW_STATUS_CONTEXT }, leaseId, leaseEpoch });
        return { written: true, status: payload };
    }
    return { set };
}
//# sourceMappingURL=github-review-status.js.map