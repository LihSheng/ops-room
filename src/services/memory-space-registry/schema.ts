...
export type MemorySpaceKind = 'project' | 'shared' | 'private-agent' | 'archive';
export type MemoryWritePolicy = 'read-only' | 'review-required';

export type MemorySpaceManifest = {
  schemaVersion: 1;
  key: string;
  version: string;
  displayName: string;
...
export function isValidMemorySpaceVersion(value: string) {
  return MEMORY_SPACE_SEMVER_PATTERN.test(value);
}

export function normalizePublicationPath(value: string) {
  return value.replaceAll('\\', '/').replace(/^\.\//, '').replace(/\/$/, '');
}

export function isSafePublicationPath(value: string) {
...
export function validateMemorySpaceManifest(value: unknown, source: string): MemorySpaceManifest {
  if (!isRecord(value)) {
  ...
  for (const field of Object.keys(value)) {
    ...
    if (SECRET_LOOKING_FIELD.test(field)) issues.push(`${source}: secret-looking field ${field} is not allowed`);
    else issues.push(`${source}: unknown field ${field}`);
  }
  ...
  if (typeof value.publicationPath !== 'string' || !isSafePublicationPath(value.publicationPath)) {
    ...
    const expectedRoot = KIND_ROOTS[value.kind as MemorySpaceKind];
  ...
  if (value.parentKey === value.key) issues.push(`${source}: parentKey cannot reference the same key`);
  ...
  if (!isRecord(value.provenance)) {
    ...
    for (const field of Object.keys(value.provenance)) {
      if (!PROVENANCE_FIELDS.has(field)) issues.push(`${source}: provenance contains unknown field ${field}`);
    }
    ...
    if (!Array.isArray(requiredFields) || requiredFields.some((field) => typeof field !== 'string')) {
      ...
      for (const field of requiredFields) {
        if (!MEMORY_PROVENANCE_FIELDS.has(field)) issues.push(`${source}: unsupported provenance field ${field}`);
      }
      const duplicates = duplicateValues(requiredFields as string[]);
      if (duplicates.length) issues.push(`${source}: duplicate provenance fields: ${duplicates.join(', ')}`);
    }
  ...
  return value as MemorySpaceManifest;
}
