...
export function memorySpaceManifestId(key, version) {
    return `${key}@${version}`;
}
...
function validateRelationships(manifests, issues) {
    const byKey = new Map();
...
export async function loadMemorySpaceManifests(dir) {
    const issues = [];
    const manifests = [];
    const sources = {};
    const seen = new Map();
    ...
    for (const spaceEntry of spaceEntries) {
        ...
        for (const versionEntry of versionEntries) {
            ...
            try {
                ...
                if (manifest.key !== spaceEntry.name)
                    issues.push(`${source}: directory key must match manifest key ${manifest.key}`);
                if (manifest.version !== versionEntry.name)
                    issues.push(`${source}: directory version must match manifest version ${manifest.version}`);
                const id = memorySpaceManifestId(manifest.key, manifest.version);
                const previous = seen.get(id);
                if (previous)
                    issues.push(`${source}: duplicate key/version pair also declared by ${previous}`);
                else {
                    seen.set(id, source);
                ...
                if (error instanceof SyntaxError)
                    issues.push(`${source}: malformed JSON`);
                else if (error instanceof MemorySpaceValidationError)
    ...
    if (manifests.length === 0)
        issues.push(`${dir}: no valid memory-space manifests found`);
