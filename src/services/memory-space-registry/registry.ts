...
let manifests = new Map<string, MemorySpaceManifest>();
let assignments: MemoryAccessResolution[] = [];
...
function cloneAssignment(assignment: MemoryAccessResolution): MemoryAccessResolution {
  return {
    ...assignment,
...
export async function initializeMemorySpaceRegistry({
  dir = MEMORY_SPACE_MANIFESTS_DIR,
...
export function getMemorySpaceManifest(key: string) {
...
export function getMemoryAssignmentsForAgent(agentId: string) {
...
export function getMemorySpaceRegistryStatus() {
  ...
  for (const manifest of manifests.values()) kindCounts[manifest.kind] += 1;
  return {
