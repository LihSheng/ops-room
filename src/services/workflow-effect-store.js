...
function validateId(value, field) {
    ...
    if (!SAFE_ID.test(normalized))
        throw new Error(`workflow_effect_${field}_invalid`);
    return normalized;
}
function validateEffectType(value) {
    const normalized = bounded(value, 80).toLowerCase();
    if (!SAFE_EFFECT_TYPE.test(normalized))
        throw new Error('workflow_effect_type_invalid');
    return normalized;
}
function validateResultCode(value) {
    const normalized = bounded(value, 120).toLowerCase();
    if (!SAFE_RESULT_CODE.test(normalized))
...
function effectIdentity({ workflowId, childId, effectType, idempotencyKey }) {
...
export function validateWorkflowEffect(record) {
    ...
    if (record.effect_id !== identity.effect_id)
        throw new Error('workflow_effect_identity_mismatch');
    ...
    if (!record.claimed_at || !record.updated_at)
        throw new Error('workflow_effect_timestamp_missing');
    if (record.output_sha != null && !SAFE_SHA.test(String(record.output_sha))) {
        throw new Error('workflow_effect_output_sha_invalid');
    }
    if (record.result_code != null)
        validateResultCode(record.result_code);
...
export async function readWorkflowEffect({ dir, effectId }) {
    try {
...
export async function claimWorkflowEffect({ dir, workflowId, childId, effectType, idempotencyKey, payload, now = () => new Date().toISOString(), }) {
    ...
    if (existing) {
        if (existing.payload_hash !== payload_hash)
            throw new Error('workflow_effect_payload_conflict');
    ...
    try {
        ...
        if (!raced)
            throw new Error('workflow_effect_claim_race_unresolved');
        if (raced.payload_hash !== payload_hash)
            throw new Error('workflow_effect_payload_conflict');
        return { created: false, effect: raced, execute: false };
    }
}
export async function completeWorkflowEffect({ dir, effectId, state = 'completed', resultCode = 'ok', outputSha = null, now = () => new Date().toISOString(), }) {
    if (!TERMINAL_STATES.has(state))
        throw new Error('workflow_effect_terminal_state_invalid');
    const current = await readWorkflowEffect({ dir, effectId });
    if (!current)
        throw new Error('workflow_effect_not_found');
    ...
    if (current.state !== 'claimed')
        throw new Error('workflow_effect_not_claimed');
    const at = now();
    const effect = validateWorkflowEffect({
        ...current,
        state,
...
async function scanWorkflowEffects({ dir }) {
    let names;
    try {
...
export async function reconcileInterruptedWorkflowEffects({ dir, now = () => new Date().toISOString(), }) {
    await mkdir(dir, { recursive: true });
...
export async function listWorkflowEffects({ dir, workflowId = null, childId = null, limit = 200 }) {
...
//# sourceMappingURL=workflow-effect-store.js.map