...
import { unknownRuntimeStatus, } from './types.js';
...
function validateAdapters(adapters) {
    ...
    for (const adapter of adapters) {
        ...
        if (ids.has(adapter.id))
            throw new Error(`Duplicate runtime adapter ID: ${adapter.id}`);
        ids.add(adapter.id);
    }
}
function selectAdapter(definition, adapters) {
    const matches = adapters.filter((adapter) => adapter.supports(definition));
    if (matches.length === 0)
        throw new Error(`No runtime adapter supports agent ${definition.key}`);
    if (matches.length > 1)
        throw new Error(`Multiple runtime adapters support agent ${definition.key}`);
    return matches[0];
}
export function prepareAgentRuntimes({ definitions = AGENT_DEFINITIONS, adapters = DEFAULT_RUNTIME_ADAPTERS, } = {}) {
    validateAdapters(adapters);
    return definitions.map((definition) => {
        const adapter = selectAdapter(definition, adapters);
        return {
            definition,
            adapter,
...
export function inspectAgentRuntimes({ definitions = AGENT_DEFINITIONS, adapters = DEFAULT_RUNTIME_ADAPTERS, } = {}) {
