...
function normalizeContainerNames(containerNames) {
    return [...new Set(containerNames.map((name) => String(name || '').trim()).filter(Boolean))].sort();
}
function parseInspectOutput(raw, containerNames) {
...
export function createDockerReadInspector({ execFile = execFileSync, now = () => Date.now(), cacheMs = DEFAULT_CACHE_MS, } = {}) {
        ...
        inspect(containerNames) {
            ...
            if (!available) {
                ...
                try {
                    ...
                    statusByContainer = Object.fromEntries(names.map((name) => [name, unknownRuntimeStatus()]));
                }
            }
            const result = {
                available,
...
//# sourceMappingURL=docker-read-inspector.js.map