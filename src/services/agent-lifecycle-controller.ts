...
let lifecycleActionQueue: Promise<void> = Promise.resolve();

export async function withLifecycleActionLock<T>(execute: () => Promise<T>): Promise<T> {
...
function observedRuntime(snapshot: Record<string, unknown> | null, agentId: string) {
  const instance = ((snapshot?.instances as Array<Record<string, unknown>>) || []).find((entry) => (
    entry?.agent === agentId || entry?.agent_id === agentId || entry?.definition?.key === agentId
  ));
  const r = instance?.runtime as Record<string, unknown> | undefined;
  return {
    adapter_id: (instance?.adapter_id as string) || null,
    status: (r?.status as string) || 'unknown',
    health: (r?.health as string) || 'unknown',
    state: (r?.state as string) || 'unknown',
  };
}

function inspectObservedRuntime(
  getRuntimeSnapshot: () => Record<string, unknown> | null,
  agentId: string,
  fallbackAdapterId: string | null = null,
) {
  try {
    return observedRuntime(getRuntimeSnapshot(), agentId);
  } catch {
    return { adapter_id: fallbackAdapterId, status: 'unknown' as const, health: 'unknown' as const, state: 'unknown' as const };
  }
}
...
async function waitForAgentDrain({
  ...
  timeoutMs: number;
  pollMs: number;
  scanTasks: (args: { dir: string }) => Promise<{ tasks?: Array<Record<string, unknown>>; corrupt?: unknown[] }>;
  now: () => number;
  sleep: (ms: number) => Promise<void>;
}) {
  const startedAt = now();
  let initialActive: number | null = null;
  while (true) {
    ...
    const active = (scanned.tasks || []).filter((task) => (
      task.agent === agentId && ACTIVE_TASK_STATES.has(task.state as string)
    ));
...
function confirmationFrom(body: Record<string, unknown>, agentId: string) {
...
function normalizeAllowedAgents(value: unknown): Set<string> {
...
export async function stopAgent({
  ...
  getRuntimeSnapshot: () => Record<string, unknown> | null;
  prepareTarget: (id: string) => { controller: { id: string; stop: (prepared: unknown, opts: { timeoutSeconds: number }) => Promise<{ controller_id: string; action: string }> }; prepared: unknown; runtime_adapter_id: string };
  scanTasks: (args: { dir: string }) => Promise<{ tasks?: Array<Record<string, unknown>>; corrupt?: unknown[] }>;
  now?: () => number;
  nowIso?: () => string;
  sleep?: (ms: number) => Promise<void>;
  ...
  let idempotencyKey: string | null = body?.idempotency_key ? String(body.idempotency_key).trim() : null;
  ...
  try {
    ...
    return rejected({
      auditDir, actor, agentId: rawAgentId, reason, idempotencyKey,
      errorCode: 'invalid_request', status: 400, message: (error as Error).message,
    });
  }
  ...
  try {
    const result = await executeIdempotent({
      ...
      execute: () => withLifecycleActionLock(() => withAgentLifecycleGate(rawAgentId, async () => {
        ...
        let target: { controller: { id: string; stop: (prepared: unknown, opts: { timeoutSeconds: number }) => Promise<{ controller_id: string; action: string }> }; prepared: unknown; runtime_adapter_id: string };
        try {
        ...
        let controllerResult: { controller_id: string; action: string } = { controller_id: target.controller.id, action: 'stop' };
        if (commandExecuted) {
          try {
        ...
        const stopEvent = await appendAuditEvent({
          dir: auditDir,
          operation: 'agent.stop',
          actor,
          target: { type: 'agent', id: rawAgentId },
        ...
        return {
          ...
          body: {
            ...
            command_executed: commandExecuted,
            audit_event_id: stopEvent.event_id,
          },
        };
      })),
    });
    ...
    return {
      status: (result.response as { status: number }).status,
      body: { ...(result.response as { body: Record<string, unknown> }).body, idempotent_replay: result.replayed },
    };
  } catch (error: unknown) {
...
export async function startAgent({
  ...
  prepareTarget: (id: string) => { controller: { id: string; start: (prepared: unknown, opts: { timeoutSeconds: number }) => Promise<{ controller_id: string; action: string }> }; prepared: unknown; runtime_adapter_id: string };
  scanTasks: (args: { dir: string }) => Promise<{ tasks?: Array<Record<string, unknown>>; corrupt?: unknown[] }>;
  now?: () => number;
  nowIso?: () => string;
  sleep?: (ms: number) => Promise<void>;
  ...
  try {
    const result = await executeIdempotent({
      ...
      execute: () => withLifecycleActionLock(() => withAgentLifecycleGate(rawAgentId, async () => {
        ...
        let target: { controller: { id: string; start: (prepared: unknown, opts: { timeoutSeconds: number }) => Promise<{ controller_id: string; action: string }> }; prepared: unknown; runtime_adapter_id: string };
        try {
          target = prepareTarget(rawAgentId);
        } catch {
          return rejected({
        ...
        let controllerResult: { controller_id: string; action: string } = { controller_id: target.controller.id, action: 'start' };
        ...
        const startEvent = await appendAuditEvent({
          dir: auditDir,
          operation: 'agent.start',
          actor,
          target: { type: 'agent', id: rawAgentId },
        ...
        return {
          ...
          body: {
            ...
            command_executed: commandExecuted,
            convergence,
            audit_event_id: startEvent.event_id,
          },
        };
      })),
    });
