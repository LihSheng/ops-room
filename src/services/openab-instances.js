...
export function getOpenABInstances({ getRuntimeSnapshot = inspectAgentRuntimes, getLifecycleState = readAgentLifecycleStateSync, lifecycleDir = LIFECYCLE_DIR, } = {}) {
    const snapshot = getRuntimeSnapshot();
    const inspectedByAgent = new Map((snapshot.instances || []).map((instance) => [instanceAgentId(instance), instance]));
    const instances = AGENT_DEFINITIONS.map((entry) => {
        ...
        const containerName = prepared?.target?.kind === 'docker-container'
            ...
            : entry.containerName;
        const convergence = classifyConvergence(lifecycle.desired_state || prepared?.desired_state || entry.desiredState, lifecycle.phase, runtime.status || 'unknown', runtime.health);
        return {
            agent: entry.key,
