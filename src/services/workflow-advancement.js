...
import { FEATURE_DEVELOPMENT_STAGES, ensureWorkflowChild, readWorkflowRun, validateWorkflowRun, } from './workflow-run-store.js';
...
function boundedReason(value, fallback = 'workflow_advancement_failed') {
...
function normalizeReviewDecision(value) {
    ...
    if (!REVIEW_DECISIONS.has(normalized))
        throw new Error('workflow_review_decision_invalid');
    return normalized;
}
...
export function planWorkflowAdvancement(record) {
    const run = validateWorkflowRun(record);
    if (TERMINAL_RUN_STATES.has(run.state)) {
        return { action: 'terminal', run };
    }
    ...
    if (activeChildren.length > 1) {
        return { action: 'escalate', reason: 'workflow_active_children_ambiguous' };
    }
    if (activeChildren.length === 1) {
        return { action: 'wait', reason: 'workflow_child_active', child: activeChildren[0] };
    }
    const pendingChildren = children.filter((child) => child.state === 'pending');
    if (pendingChildren.length > 1) {
        return { action: 'escalate', reason: 'workflow_pending_children_ambiguous' };
    }
    if (pendingChildren.length === 1) {
        return { action: 'execute', child: pendingChildren[0] };
    }
    ...
    if (lastChild.state !== 'completed' || !SAFE_SHA.test(String(lastChild.output_sha || ''))) {
        return { action: 'escalate', reason: 'workflow_completed_child_evidence_invalid', child: lastChild };
    }
    const stageIndex = FEATURE_DEVELOPMENT_STAGES.indexOf(lastChild.stage);
    if (stageIndex < 0) {
        return { action: 'escalate', reason: 'workflow_stage_invalid', child: lastChild };
    }
    ...
    try {
        ...
        return { action: 'escalate', reason: 'workflow_review_decision_missing', child: lastChild };
    }
    if (decision === 'approved') {
        return { action: 'complete', child: lastChild };
    }
    if (lastChild.iteration >= run.policy.max_iterations) {
        return { action: 'escalate', reason: 'workflow_iteration_limit_exceeded', child: lastChild };
    }
    return {
        action: 'create',
...
async function persistReviewDecision({ workflowRunsDir, workflowId, childId, evidence, now, }) {
    ...
    if (current.stage !== 'review')
        throw new Error('workflow_review_child_required');
    if (current.state !== 'completed')
        throw new Error('workflow_review_child_incomplete');
    const normalized = normalizeReviewEvidence(evidence);
    if (current.review_decision) {
        if (current.review_decision !== normalized.decision
...
async function persistTerminalRun({ workflowRunsDir, workflowId, state, reason, now }) {
    const run = await readWorkflowRun({ dir: workflowRunsDir, workflowId });
    ...
    if (TERMINAL_RUN_STATES.has(run.state))
        throw new Error('workflow_terminal_state_immutable');
    const at = now();
    const updated = validateWorkflowRun({
        ...run,
...
export async function advanceWorkflowRun({ workflowRunsDir, workflowId, executeChild, resolveReviewDecision = null, ensureChild = ensureWorkflowChild, readRun = readWorkflowRun, maxSteps = DEFAULT_MAX_STEPS, advancementLockDir = join(workflowRunsDir, '.advancement-locks'), advancementLockTimeoutMs = 10_000, advancementLockStaleAfterMs = DEFAULT_ADVANCEMENT_LOCK_STALE_AFTER_MS, now = () => new Date().toISOString(), }) {
    if (!workflowRunsDir || !workflowId)
        throw new Error('workflow_advancement_input_invalid');
    if (typeof executeChild !== 'function')
        throw new Error('workflow_advancement_executor_required');
        ...
        execute: async () => {
            for (let step = 1; step <= boundedMaxSteps; step += 1) {
                ...
                if (plan.child.stage === 'review') {
                    ...
                    const reviewChild = refreshed.children.find((child) => child.child_id === plan.child.child_id);
...
//# sourceMappingURL=workflow-advancement.js.map