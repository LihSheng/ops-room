...
import { reconcileReviewDecisionFromEffects, } from './workflow-provider-advancement.js';
...
function childEffectType(child) {
...
function validateWorkspaceOwnership({ run, child, record }) {
    ...
    if ((record.branch || null) !== (binding.branch || null))
        throw new Error('workflow_child_workspace_branch_mismatch');
    return record;
}
async function currentAttemptEffect({ effectsDir, workflowId, child, listEffects = listWorkflowEffects, }) {
    ...
    const matches = effects.filter((effect) => (effect.effect_type === childEffectType(child)
        && effect.idempotency_key === attemptKey(child.attempt)));
    if (matches.length > 1)
        throw new Error('workflow_recovery_effect_ambiguous');
    return matches[0] || null;
}
async function exactWorkspaceHead({ run, child, recordRoot, workspaceRoot, inspectWorkspaceHead, readRecord = readWorkspaceRecord, }) {
    ...
    if (!SAFE_SHA.test(head))
        throw new Error('workflow_recovery_workspace_head_invalid');
    return { record, cwd, head };
}
async function persistRun({ workflowRunsDir, workflowId, record }) {
    const validated = validateWorkflowRun(record);
    await writeAtomic(workflowPath(workflowRunsDir, workflowId), validated);
    return validated;
}
async function ensureRecoveredWorkspaceCleanup({ run, child, recordRoot, requestCleanup = requestWorkspaceCleanup, readRecord = readWorkspaceRecord, now, }) {
    const record = validateWorkspaceOwnership({
        run,
        child,
        record: await readRecord({ dir: recordRoot, workspaceId: child.workspace.workspace_id }),
...
async function finalizeRecoveryCleanup({ workflowRunsDir, workflowId, run, child, recordRoot, requestCleanup, readRecord, now, }) {
    ...
    const at = now();
    const index = run.children.findIndex((candidate) => candidate.child_id === child.child_id);
    if (index < 0)
...
async function reconcileOneWorkflow({ workflowRunsDir, workflowId, effectsDir, workspaceRoot, recordRoot, inspectWorkspaceHead, requestCleanup, readRun, listEffects, readRecord, now, }) {
    return withWorkspaceLock({
        ...
        execute: async () => {
            ...
            const pendingCleanup = run.children.filter((child) => (child.state === 'completed' && child.recovery_cleanup_pending === true));
            if (pendingCleanup.length > 1)
                throw new Error('workflow_recovery_cleanup_ambiguous');
            if (pendingCleanup.length === 1) {
            ...
            const interrupted = run.children.filter((child) => (child.state === 'needs_human' && child.last_error === INTERRUPTED_CHILD_REASON));
            if (interrupted.length === 0) {
                return { changed: false, recovered_child: null, cleanup_reconciled: null, review_child: null };
            }
            if (interrupted.length > 1)
                throw new Error('workflow_recovery_interrupted_children_ambiguous');
            ...
            if (!SAFE_SHA.test(outputSha))
                throw new Error('workflow_recovery_effect_output_sha_invalid');
            ...
            if (workspace.head !== outputSha)
                throw new Error('workflow_recovery_workspace_head_mismatch');
            const at = now();
            const index = run.children.findIndex((candidate) => candidate.child_id === child.child_id);
...
export async function inspectGitWorkspaceHead({ cwd, execFile = execFileDefault, }) {
    try {
        ...
        if (error?.message === 'workflow_recovery_workspace_head_invalid')
            throw error;
        throw new Error('workflow_recovery_workspace_head_unavailable');
    }
}
export async function reconcileProviderBackedWorkflowRuns({ workflowRunsDir, effectsDir, workspaceRoot, recordRoot, inspectWorkspaceHead = inspectGitWorkspaceHead, requestCleanup = requestWorkspaceCleanup, readRun = readWorkflowRun, listRuns = listWorkflowRuns, listEffects = listWorkflowEffects, readRecord = readWorkspaceRecord, reconcileReviewDecision = reconcileReviewDecisionFromEffects, now = () => new Date().toISOString(), }) {
    ...
    for (const record of records) {
        ...
        try {
            ...
            if (result.recovered_child)
                recovered.push(result.recovered_child);
            if (result.cleanup_reconciled)
                cleanupReconciled.push(result.cleanup_reconciled);
            ...
            if (decision?.reconciled)
                reviewDecisions.push(decision.child.child_id);
        }
        catch {
...
export async function retryWorkflowChildAfterInvestigation({ workflowRunsDir, workflowId, childId, expectedAttempt, effectsDir, workspaceRoot, recordRoot, inspectWorkspaceHead = inspectGitWorkspaceHead, readRun = readWorkflowRun, listEffects = listWorkflowEffects, readRecord = readWorkspaceRecord, updateRecord = updateWorkspaceRecord, now = () => new Date().toISOString(), }) {
    ...
    return withWorkspaceLock({
        ...
        execute: async () => {
            ...
            if (run.state !== 'needs_human')
                throw new Error('workflow_retry_run_not_needs_human');
            if (child.state !== 'needs_human')
                throw new Error('workflow_retry_child_not_needs_human');
            if (child.attempt !== normalizedExpectedAttempt)
                throw new Error('workflow_retry_attempt_conflict');
            const effect = await currentAttemptEffect({ effectsDir, workflowId, child, listEffects });
            if (!effect)
                throw new Error('workflow_retry_effect_unavailable');
            ...
            if (workspace.head !== child.input_sha)
                throw new Error('workflow_retry_workspace_head_changed');
...
export async function resumePendingWorkflowAfterInvestigation({ workflowRunsDir, workflowId, childId, expectedAttempt, effectsDir, workspaceRoot, recordRoot, inspectWorkspaceHead = inspectGitWorkspaceHead, readRun = readWorkflowRun, listEffects = listWorkflowEffects, readRecord = readWorkspaceRecord, now = () => new Date().toISOString(), }) {
    const normalizedExpectedAttempt = Number(expectedAttempt);
    attemptKey(normalizedExpectedAttempt);
    return withWorkspaceLock({
        dir: join(workflowRunsDir, '.advancement-locks'),
        ...
        execute: async () => {
            ...
            if (run.state !== 'needs_human')
                throw new Error('workflow_resume_run_not_needs_human');
            if (child.state !== 'pending')
                throw new Error('workflow_resume_child_not_pending');
            if (child.attempt !== normalizedExpectedAttempt)
                throw new Error('workflow_resume_attempt_conflict');
            const effect = await currentAttemptEffect({ effectsDir, workflowId, child, listEffects });
            if (effect)
                throw new Error('workflow_resume_provider_effect_exists');
            if (child.workspace?.workspace_id) {
                ...
                if (workspace.record.state !== 'active')
                    throw new Error('workflow_resume_workspace_not_active');
                if (workspace.head !== child.input_sha)
                    throw new Error('workflow_resume_workspace_head_changed');
            }
            const at = now();
            const updated = await persistRun({
                workflowRunsDir,
...
export { INTERRUPTED_CHILD_REASON, RECOVERY_CLEANUP_PENDING, };
//# sourceMappingURL=workflow-provider-recovery.js.map