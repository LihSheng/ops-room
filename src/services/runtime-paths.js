...
export const OPERATOR_CONFIGURED_ROLES = Object.freeze(String(process.env.OPS_ROOM_OPERATOR_ROLES || '')
    .split(',')
    .map((value) => value.trim())
    .filter(Boolean));
export const OPERATOR_SESSION_TTL_SECONDS = boundedInteger(process.env.OPS_ROOM_OPERATOR_SESSION_TTL_SECONDS, 8 * 60 * 60, 300, 7 * 24 * 60 * 60);
...
export const AGENT_LIFECYCLE_ALLOWED_AGENTS = Object.freeze(String(process.env.OPS_ROOM_AGENT_LIFECYCLE_ALLOWED_AGENTS || '')
    .split(',')
    .map((value) => value.trim())
    .filter(Boolean));
