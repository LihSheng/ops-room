...
import { clearOperatorSessionCookie, createOperatorSession, extractOperatorSessionToken, readOperatorSession, revokeOperatorSession, serializeOperatorSessionCookie, } from '../services/operator-session-store.js';
import { deriveOperatorCsrfToken, verifyOperatorCsrfToken, } from '../services/operator-request-auth.js';
import { AUDIT_DIR, HUMAN_AUTH_ENABLED, OPERATOR_CONFIGURED_ROLES, OPERATOR_SESSION_COOKIE_SECURE, OPERATOR_SESSION_DIR, OPERATOR_SESSION_TTL_SECONDS, } from '../services/runtime-paths.js';
function hidden() {
...
export async function handleCreateOperatorSession({ authorization, enabled = HUMAN_AUTH_ENABLED, sessionDir = OPERATOR_SESSION_DIR, auditDir = AUDIT_DIR, roles = OPERATOR_CONFIGURED_ROLES, ttlSeconds = OPERATOR_SESSION_TTL_SECONDS, secureCookie = OPERATOR_SESSION_COOKIE_SECURE, verifyBootstrapAuth = verifyOperatorBootstrapAuth, resolveActor = resolveOperatorIdentity, createSession = createOperatorSession, revokeSession = revokeOperatorSession, appendAudit = appendAuditEvent, now, }) {
    if (!enabled)
        return hidden();
    if (!verifyBootstrapAuth(authorization))
        return unauthorized();
    let created = null;
    try {
...
export async function handleReadOperatorSession({ cookieHeader, enabled = HUMAN_AUTH_ENABLED, sessionDir = OPERATOR_SESSION_DIR, readSession = readOperatorSession, now, }) {
    if (!enabled)
        return hidden();
...
export async function handleRevokeOperatorSession({ cookieHeader, csrfHeader, enabled = HUMAN_AUTH_ENABLED, sessionDir = OPERATOR_SESSION_DIR, auditDir = AUDIT_DIR, secureCookie = OPERATOR_SESSION_COOKIE_SECURE, readSession = readOperatorSession, revokeSession = revokeOperatorSession, appendAudit = appendAuditEvent, now, }) {
    if (!enabled)
        return hidden();
    const clearHeader = {
    ...
    const token = extractOperatorSessionToken(cookieHeader);
    if (!token)
        return unauthorized(clearHeader);
...
//# sourceMappingURL=operator-sessions.js.map