...
import { pauseTask, readTask, requestCancellation, resumeTask, retryTask, } from '../services/review-task-store.js';
...
async function withTaskActionLock(taskId, execute) {
    ...
    try {
        ...
        release();
        if (taskActionLocks.get(taskId) === queued)
            taskActionLocks.delete(taskId);
    }
}
function transitionErrorCode(error) {
    const message = String(error?.message || '');
    if (/retry budget exhausted/i.test(message))
        return 'retry_budget_exhausted';
    return 'invalid_transition';
}
async function rejected({ auditDir, actor, operation, taskId, reason, idempotencyKey, errorCode, status, message, previousState = null, }) {
...
export async function handleOperatorTaskAction({ action, taskId, body, actor, reviewTasksDir, auditDir, idempotencyDir, }) {
...
//# sourceMappingURL=operator-tasks.js.map