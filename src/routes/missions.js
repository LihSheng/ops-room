import { listMissions, readMission, serializeMission, } from '../services/mission-store.js';
...
function normalizeStateFilter(value) {
    ...
    if (!state)
        return null;
    if (!MISSION_STATES.has(state))
        throw new Error('invalid_mission_state_filter');
    return state;
}
function normalizePriorityFilter(value) {
    ...
    if (!PRIORITIES.has(priority))
        throw new Error('invalid_mission_priority_filter');
    return priority;
}
function unavailableMission(record, missionId = null) {
...
function serializeForRead(record, includeHistory) {
...
export async function handleMissionsList(searchParams, { missionsDir, listRecords = listMissions, }) {
    ...
    const filtered = serialized.filter((mission) => {
        if (repository && mission.repository_id !== repository)
            return false;
...
export async function handleMissionDetail(missionId, { missionsDir, readRecord = readMission, }) {
