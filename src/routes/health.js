...
import { TASKS_DIR, REVIEW_TASKS_DIR, WORKFLOW_RUNS_DIR, WORKFLOW_EFFECTS_DIR, STATE_DIR, LOG_DIR, WORKSPACE_BASE, AUDIT_DIR, IDEMPOTENCY_DIR, LIFECYCLE_DIR, AGENT_PROFILES_DIR, MEMORY_SPACE_MANIFESTS_DIR, OPENAB_SERVER_VERSION, REQUIRED_COMMANDS, } from '../services/runtime-paths.js';
...
async function getCommandStatus(commandExistsFn = commandExists, requiredCommands = REQUIRED_COMMANDS) {
    ...
    if (commandExistsFn !== commandExists) {
        return Object.fromEntries(await Promise.all(commandNames.map(async (name) => [name, await commandExistsFn(name)])));
    }
    const now = Date.now();
    if (cachedCommandStatus && now - cachedAt < COMMAND_CACHE_MS) {
        return cachedCommandStatus;
    }
    cachedCommandStatus = Object.fromEntries(await Promise.all(commandNames.map(async (name) => [name, await commandExistsFn(name)])));
    cachedAt = now;
    return cachedCommandStatus;
}
...
export async function handleHealth({ commandExistsFn = commandExists, directoryCheckFn = checkDirectory, lifecycle = processLifecycle, releaseInfoFn = readReleaseInfo, profileStatusFn = getAgentProfileRegistryStatus, skillStatusFn = getSkillRegistryStatus, memoryStatusFn = getMemorySpaceRegistryStatus, requiredCommands = REQUIRED_COMMANDS, } = {}) {
    let releaseInfo;
    let releaseIdentity;
    try {
