...
import {
  REPO, SHARED_MEMORY, REPOSITORY_CACHE_ROOT, TASK_WORKSPACE_ROOT,
  WORKSPACE_RECORDS_DIR, WORKSPACE_LOCK_DIR, WORKSPACE_MAX_ACTIVE, WORKSPACE_MIN_FREE_BYTES,
...
async function bindReviewWorkspace({ dir, taskId, headSha, lease }) {
  if (!dir || !taskId) return null;
  let task = await readTask({ dir, id: taskId });
  if (!task) throw new Error('review_task_unavailable');
...
export async function runPrReviewWorkflow(payload) {
  const {
    agent, task, task_type, repository, pr, mode, commenter = 'unknown',
    comment_id, head_sha, task_id, dir, lease,
  } = payload;
  ...
  if (!reviewText) {
    ...
    if (!retryText) throw new Error(`PR review generation returned an empty response for ${repository}#${pr} (retried once)`);
  ...
  if (responseMode === 'chat') {
    if (dir && task_id) {
      ...
      if (!effect.claimed) {
        if (effect.state === 'COMPLETED') return { mode: 'pr_chat', repository, pr, agent, review_event: 'COMMENT', duplicate_effect: true };
        if (effect.state === 'CLAIMED') return { mode: 'pr_chat', repository, pr, agent, review_event: 'NEEDS_HUMAN', ambiguous_effect: true, effect_id: effect.effect?.id };
        const reclaimed = await reclaimEffect({ dir, effectId: effect.effect.id, leaseId: lease?.lease_id, leaseEpoch: lease?.lease_epoch });
        if (!reclaimed.reclaimed) return { mode: 'pr_chat', repository, pr, agent, review_event: 'NEEDS_HUMAN', ambiguous_effect: true, effect_id: effect.effect?.id };
    ...
    if (dir && task_id) {
      ...
      if (!effect.claimed) {
        if (effect.state === 'COMPLETED') return { mode: 'pr_review', repository, pr, agent, review_event: event, structured_review: structured, duplicate_effect: true };
        if (effect.state === 'CLAIMED') return { mode: 'pr_review', repository, pr, agent, review_event: 'NEEDS_HUMAN', structured_review: structured, ambiguous_effect: true, effect_id: effect.effect?.id };
        const reclaimed = await reclaimEffect({ dir, effectId: effect.effect.id, leaseId: lease?.lease_id, leaseEpoch: lease?.lease_epoch });
        if (!reclaimed.reclaimed) return { mode: 'pr_review', repository, pr, agent, review_event: 'NEEDS_HUMAN', structured_review: structured, ambiguous_effect: true, effect_id: effect.effect?.id };
        effect.effect = reclaimed.effect;
      }
