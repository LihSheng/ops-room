...
import { addComment, ghApi, ghApiText, ensureLabel, removeLabel, addLabel, transitionLabels, githubToken } from '../services/github.js';
import { REPO, WORKSPACE_BASE, FORBIDDEN_FILE_PATTERNS, LOCK_DIR, PROMPT_DIR } from '../services/runtime-paths.js';
...
import { loadProcessedTasks, markTaskProcessed, acquireLock, releaseLock, ensureDir, fileExists } from '../services/task-store.js';
...
function getDefaultBranch(env) {
    return execSync(`gh repo view ${REPO} --json defaultBranchRef --jq .defaultBranchRef.name`, { encoding: 'utf-8', env }).trim() || 'main';
}
// ── Branch name ─────────────────────────────────────────────────────────────
...
async function execLogged(command, ctx, opts = {}) {
    ...
    try {
        ...
        if (allowFailure)
            return { ok: false, stdout: '', error: safeMsg };
...
function runCommandWithStdin({ command, args, cwd, stdin, env = process.env, timeoutMs, ctx }) {
    return new Promise((resolve, reject) => {
        ...
        if (ctx)
            registerAgentProcess(ctx, child);
        let stdout = '';
        let stderr = '';
        let settled = false;
...
function validateChangedFiles(ctx, changedFiles) {
    ...
    if (suspicious.length > 0) {
        throw new Error(`Suspicious root-level artifact files detected: ${suspicious.join(', ')}. ` +
            `The agent likely wrote scratch/output files into the repo. Refusing to commit.`);
    }
    const realSourceOrTestChanges = changedFiles.filter((file) => file.startsWith('src/') ||
        file.startsWith('app/') ||
    ...
    if (realSourceOrTestChanges.length === 0) {
        throw new Error(`No real source/test changes detected. Changed files: ${changedFiles.join(', ')}`);
    }
}
...
async function pushBranch(ctx) {
    ...
    const gitEnv = { ...process.env, ...askpass.env };
    try {
        const result = await execLogged(`cd "${ctx.workspaceDir}" && git push origin HEAD:refs/heads/"${ctx.branchName}"`, ctx, { allowFailure: true, env: gitEnv });
        if (!result.ok) {
            const error = result.error || '';
            if (error.includes('non-fast-forward') ||
                error.includes('fetch first') ||
                error.includes('Updates were rejected')) {
                throw new Error(`Push rejected because remote branch already exists or local branch is stale. ` +
...
async function verifyPrExists(ctx) {
    const token = githubToken(ctx.agent);
    const env = { ...process.env, GH_TOKEN: token };
    const output = execCapture(`gh pr list --repo "${REPO}" --head "${ctx.branchName}" --json url --jq '.[0].url'`, env);
...
async function handleCodingFailure(ctx, error) {
    ...
    if (isCommandFailure)
        failureType = 'coding command failed';
    else if (isNoChangeFailure)
        failureType = 'no source changes detected';
...
I will create a branch, make changes, run checks, and open a PR. This task will not be marked done unless a PR URL exists.`);
        ...
        const suspiciousFiles = changedFiles.filter(f => f.startsWith('.openab/') || f === 'TASK.md' || f.endsWith('.task.md') || (/^[^/]+\.md$/.test(f) && f !== 'README.md'));
        if (suspiciousFiles.length > 0 && suspiciousFiles.length === changedFiles.length) {
            throw new Error(`Agent produced only task/markdown files and no real app/source changes. ` +
...
async function handleTask(issueNumber, agentKey) {
    ...
    try {
        ...
        const isPrReviewTask = Boolean(issue.pull_request && (labelNames.includes('openab/pr-review') ||
        ...
        try {
            ...
            if (flag === 'code')
                ...
                isCoding = false;
            else if (metadataTaskType === 'code')
                isCoding = true;
            else if (metadataTaskType === 'chat')
                isCoding = false;
            else
                isCoding = isCodingTask(taskDesc, issue);
            let codingResult;
            if (isPrReviewTask) {
...
export { commandExists, prepareWorkspace, excludeHarnessFilesFromGit, writeTaskPrompt, runCodingWorkflow, runCodingAgent, collectGitDebugSnapshot, validateCodingWorkspace, execCapture, buildContext, handleTask, buildBranchName, cancelTask, registerAgentProcess, commitIfChanges, pushBranch, hasGitChanges, getChangedFiles, getGitDiffStat, configureGitAuthor, execLogged, createAskpassHelper, cleanupAskpassHelper, buildAskpassScriptPath, buildAgentEnv, maskToken, renderAskpassScript, };
