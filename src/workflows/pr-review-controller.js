...
import { agentLifecycleAllowsDispatch, readAgentLifecycleState, withAgentLifecycleGate, } from '../services/agent-lifecycle-store.js';
import { claimTask, checkConcurrency, countActiveTasks, createOrClaimTask, readTask, transitionTask, } from '../services/review-task-store.js';
...
export function createPrReviewController({ fetchPullRequest, setCommitStatus, dispatchReview, canDispatchAgent = defaultCanDispatchAgent, withAgentDispatchGate = withAgentLifecycleGate, instanceId = `ops-room-${process.pid}`, clock = () => new Date(), }) {
    if (typeof fetchPullRequest !== 'function')
        throw new Error('fetchPullRequest is required');
    if (typeof setCommitStatus !== 'function')
        throw new Error('setCommitStatus is required');
    if (typeof dispatchReview !== 'function')
        throw new Error('dispatchReview is required');
    if (typeof canDispatchAgent !== 'function')
        throw new Error('canDispatchAgent is required');
    if (typeof withAgentDispatchGate !== 'function')
        throw new Error('withAgentDispatchGate is required');
    async function submit(request) {
        ...
        if (!currentSha)
            throw new Error(`PR ${normalized.repository}#${normalized.pr} has no head SHA`);
