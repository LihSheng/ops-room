...
import { AGENT_NAMES, POLL_AGENTS } from '../lib/config.js';
...
function githubToken(agentKey) {
    return getTokenForAgent(agentKey, join(__dirname, 'github-app-token.js'));
}
...
async function handleTask(issueNumber, agentKey, issue) {
    ...
    if (lower.includes('say hi') || lower.includes('hello') || lower.includes('hi here') || lower.includes('ping')) {
        await addComment(issueNumber, `👋 Hi @${commenter}! ${agentName} here, responding live.
