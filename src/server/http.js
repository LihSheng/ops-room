...
import { REPO, PORT, HOST, WEBHOOK_SECRET, WORKSPACE_BASE, REVIEW_TASKS_DIR, MISSIONS_DIR, WORKFLOW_RUNS_DIR, AUDIT_DIR, IDEMPOTENCY_DIR, LIFECYCLE_DIR, OPENAB_SERVER_VERSION, SHUTDOWN_TIMEOUT_MS, ISSUE_POLLING_ENABLED, HUMAN_AUTH_ENABLED, AGENT_LIFECYCLE_ENABLED, AGENT_LIFECYCLE_ALLOWED_AGENTS, AGENT_LIFECYCLE_DRAIN_TIMEOUT_MS, AGENT_LIFECYCLE_DRAIN_POLL_MS, AGENT_LIFECYCLE_STOP_TIMEOUT_SECONDS, AGENT_LIFECYCLE_START_TIMEOUT_SECONDS, } from '../services/runtime-paths.js';
import { initDirs } from '../services/task-store.js';
import '../services/logs.js';
import { addComment, ensureLabel, removeLabel, addLabel, getCommitStatuses, createCommitStatus, ghApi } from '../services/github.js';
...
import { handleOperatorTaskAction } from '../routes/operator-tasks.js';
import { canDispatchAgentFromLifecycle, handleOperatorAgentStart, handleOperatorAgentStop, } from '../routes/operator-agents.js';
...
import { handleCreateOperatorSession, handleReadOperatorSession, handleRevokeOperatorSession, } from '../routes/operator-sessions.js';
import { handleOperatorSessionAdministrativeRevoke, handleOperatorSessionsList, } from '../routes/operator-session-administration.js';
...
import { authorizeOperatorRequest, OPERATOR_CSRF_HEADER_NAME, } from '../services/operator-request-auth.js';
...
const dashboardReadGuard = async (req, res) => {
...
const drainGuard = async (req, res) => {
    ...
    try {
        ...
        if (state === 'CHANGES_REQUESTED' && task.mode === 'auto-fix') {
            ...
            if (autoFixPolicy.allowed) {
                ...
                scheduleTracked(`fix:${child.task.id}`, () => executeControllerFix({ dir: task.dir, taskId: child.task.id }), '[fix-child-controller] unhandled execution error:');
            }
            else {
                await transitionTask({
                    dir: task.dir,
...
configurePrReviewController(createPrReviewController({
    ...
    dispatchReview: (task) => scheduleTracked(`review:${task.task_id || task.id || task.headSha}`, () => executeControllerReview(task), '[pr-review-controller] unhandled execution error:'),
}));
    ...
    setImmediate(() => {
        dispatchEligibleTasks({
            ...
            for (const task of result.tasks) {
                if (task.kind === 'fix') {
                    scheduleTracked(`operator:${task.id}`, () => executeControllerFix({
                        dir: REVIEW_TASKS_DIR,
                        taskId: task.id,
                    ...
                    scheduleTracked(`operator:${task.id}`, () => executeControllerReview({
                        dir: REVIEW_TASKS_DIR,
                        task_id: task.id,
                        task: task.task_text,
...
const routes = [
    ...
    },
    {
        ...
        handler: async (_req, res, params) => {
            ...
            if (!task) {
                sendJSON(res, 404, { error: 'Review task not found' });
                return;
            }
    ...
    },
    {
        ...
        handler: async (req, res, params) => {
            ...
            if (!event)
                sendJSON(res, 404, { error: 'Audit event not found' });
            else
...
async function listOpenIssuesForAgent(agentKey) {
    ...
    for (const labelQuery of labelQueries) {
        try {
            const out = execSync(`gh api repos/${REPO}/issues?labels=${encodeURIComponent(labelQuery)}&state=open&per_page=100&sort=created&direction=desc`, { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
...
const issuePollerPromise = ISSUE_POLLING_ENABLED ? processLifecycle.track(startIssuePoller({
    ...
    pollAgent: (agentKey, signal) => pollAgentIssues({
        ...
        addLabel,
        addComment,
        handleTask: (issueNumber, agentKey, issue) => trackAcceptedOperation(processLifecycle, `legacy-issue:${agentKey}#${issueNumber}`, () => handleTask(issueNumber, agentKey, issue)),
        cancelTask,
        signal,
    }),
}), 'issue-poller') : Promise.resolve();
issuePollerPromise.catch((e) => console.error('[server] poller fatal:', e.message));
if (!ISSUE_POLLING_ENABLED)
    console.log('[poller] disabled by OPS_ROOM_ISSUE_POLLING_ENABLED=false');
...
server.listen(PORT, HOST, () => {
    ...
    if (AGENT_LIFECYCLE_ENABLED)
        console.log(`  POST /api/operator/agents/:agent/stop - Guarded graceful stop`);
    if (AGENT_LIFECYCLE_ENABLED)
        console.log(`  POST /api/operator/agents/:agent/start - Guarded graceful start`);
