...
export function createGitHubOps({ repo, tokenForAgent, processEnv = process.env, logger = console }) {
    ...
    function addComment(issueNumber, body, agentKey = 'professor') {
        const tryPost = (key) => {
            ...
            return execFileSync('gh', ['api', `repos/${repo}/issues/${issueNumber}/comments`, '-X', 'POST', '-f', `body=${body}`], { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024, env: { ...processEnv, GH_TOKEN: token } });
        };
        try {
    ...
    function ensureLabel(label, color = 'fbca04') {
        try {
            execSync(`gh label create ${JSON.stringify(label)} --repo ${JSON.stringify(repo)} --color ${JSON.stringify(color)} --force`, { encoding: 'utf-8', stdio: 'pipe' });
        }
        catch (error) {
